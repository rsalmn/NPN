...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (V5 IMPROVED)", TextSize = 20 })
    ...
    -- =========================================================
    -- 3. IMPROVEMENT #1: TELEPORT MANAGER (Anti-Spam & Safe)
    -- =========================================================
    local TeleportManager = {
        LastTeleport = 0,
        MinInterval = 1.2,      -- Minimal jarak waktu antar teleport
        TeleportCount = 0,
        MaxTPPerMinute = 18,    -- Max teleport per menit
        LastPosition = nil
    }
    ...
    function TeleportManager:Teleport(pos)
        ...
        if not canTP then
            warn("⚠️ Teleport blocked:", reason)
            return false
        end
        ...
        -- Tambahkan random offset untuk anti-detection
        ...
        -- Jangan teleport jika terlalu dekat dengan posisi terakhir
        if self.LastPosition then
            local distance = (finalPos - self.LastPosition).Magnitude
            if distance < 30 then
                return false -- Skip, terlalu dekat
            end
        end
        ...
        -- Auto reset counter setelah 60 detik
    ...
    -- =========================================================
    -- 4. IMPROVEMENT #2: DYNAMIC CHUNK LOADER
    -- =========================================================
    local function WaitForChunkLoad(position, timeout)
        ...
        while tick() - startTime < timeout do
            ...
            -- Jika jumlah parts stabil (tidak berubah) selama 2 checks
            if currentCount == lastPartCount then
                stableCount = stableCount + 1
                if stableCount >= 2 then
                    return true -- Chunk loaded
                end
            else
                stableCount = 0
            end
    ...
    -- =========================================================
    -- 5. IMPROVEMENT #3: OPTIMIZED LOCHNESS DETECTOR
    -- =========================================================
    local LochnessDetector = {
        LastScan = 0,
        ScanCooldown = 10,
        CachedPosition = nil,
        KnownSpawnZones = {} -- Learning spawn locations
    }
    ...
    function LochnessDetector:Find()
        ...
        -- Cek apakah Lochness window aktif
        local _, _, active = getLochNextTimes()
        if not active then
            return nil
        end
        
        -- 1. Cek known spawn zones terlebih dahulu (FAST)
        for _, zone in ipairs(self.KnownSpawnZones) do
            ...
            if success then
                for _, part in ipairs(parts) do
                    ...
                    if model and self:IsLochness(model) then
        ...
        -- 2. Check CollectionService tags (MEDIUM)
        ...
        -- 3. LIMITED workspace scan (SLOW - only as fallback)
        -- Hanya scan children workspace, BUKAN descendants
    ...
    -- =========================================================
    -- 6. IMPROVEMENT #4: MULTI-RADIUS SCANNER
    -- =========================================================
    local function IsEventAlive(model, eventName)
    ...
    local function SmartScan(position, keywords, targetName)
        -- Multi-radius approach: start small, expand if needed
        local radii = {100, 200, 400}
        ...
        for radiusIndex, radius in ipairs(radii) do
            local success, parts = pcall(function()
                return workspace:GetPartBoundsInBox(
                    CFrame.new(position), 
                    Vector3.new(radius, 200, radius), 
                    overlapParams
                )
            end)
            
            if not success then 
                warn("⚠️ Scan failed at radius", radius)
                continue 
            end
            
            -- PASS 1: Check tagged parts first (fastest)
            for _, part in ipairs(parts) do
                if part:HasTag("Event") or part:HasTag("Boss") or part:HasTag("Monster") then
                    local model = part:FindFirstAncestorOfClass("Model")
                    if model and IsEventAlive(model, targetName) then
                        return true, part.Position
                    end
                end
            end
            
            -- PASS 2: Name-based detection
            for _, part in ipairs(parts) do
            ...
            -- Jika radius kecil tidak ketemu, coba radius lebih besar
            if radiusIndex < #radii then
                task.wait(0.15)
            end
        end
    ...
    function ErrorHandler:Handle(err)
        ...
        if self.ErrorCount >= self.MaxErrors then
            WindUI:Notify({
                Title = "System Error",
                Content = "Too many errors, stopping system",
                Duration = 6,
                Icon = "alert-triangle"
            })
    ...
    -- =========================================================
    -- 8. IMPROVEMENT #6: PERFORMANCE MONITOR
    -- =========================================================
    ...
    function PerformanceMonitor:RecordProbe(success, eventName)
        self.TotalProbes = self.TotalProbes + 1
        
        if success then
            self.SuccessfulProbes = self.SuccessfulProbes + 1
            self.EventsFound[eventName] = (self.EventsFound[eventName] or 0) + 1
        else
            self.FailedProbes = self.FailedProbes + 1
        end
    end

    function PerformanceMonitor:GetStats()
        local uptime = math.floor((tick() - self.StartTime) / 60)
        local successRate = self.TotalProbes > 0 and 
            math.floor((self.SuccessfulProbes / self.TotalProbes) * 100) or 0
        
        return string.format(
            "Uptime: %dm | Probes: %d | Success: %d%% | TP Count: %d",
            uptime,
            self.TotalProbes,
            successRate,
    ...
    -- =========================================================
    -- 9. IMPROVED PROBE FUNCTION (V5 Base + All Fixes)
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        if not targetName then return false end
        
        -- LOCHNESS SPECIAL HANDLING (Improved)
        if targetName == "Lochness Hunt" then
            local pos = LochnessDetector:Find()
            if pos then
                local success = TeleportManager:Teleport(pos)
                if success then
                    print("✅ [Lochness] Found & Teleported!")
                    PerformanceMonitor:RecordProbe(true, targetName)
                    return true
                end
            else
                -- Teleport ke idle area sambil menunggu
                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                    local data = FishingAreass[Loch_Return_SelectedArea]
                    TeleportManager:Teleport(data.Pos)
                end
                print("⏳ [Lochness] Waiting for spawn window...")
            end
            PerformanceMonitor:RecordProbe(false, targetName)
            return false
        end

        -- NORMAL EVENT PROBING (Improved)
        ...
        print("🕵️ [Probe] Checking: " .. targetName)

        for i, pos in ipairs(coords) do
            -- 1. SAFE TELEPORT (dengan rate limiting)
            local tpSuccess = TeleportManager:Teleport(pos)
            if not tpSuccess then
                -- Jika gagal teleport, tunggu sebentar
                if TeleportManager.TeleportCount > TeleportManager.MaxTPPerMinute then
            ...
            local loaded = WaitForChunkLoad(pos, 2.5)
            if not loaded then
                print("⏳ [Chunk] Loading timeout at spot #" .. i)
            end
            
            -- 3. MULTI-RADIUS SMART SCAN
            local found, eventPos = SmartScan(pos, keywords, targetName)
            
            if found then
                print("✅ [FOUND] " .. targetName .. " at spot #" .. i)
                
                -- Re-adjust posisi jika event ditemukan agak jauh
                if eventPos and (eventPos - pos).Magnitude > 25 then
                    TeleportManager:Teleport(eventPos)
                end
            ...
            -- Small delay sebelum cek spot berikutnya
            task.wait(0.3)
        end

        print("❌ [NOT FOUND] " .. targetName .. " - checked all spots")
        PerformanceMonitor:RecordProbe(false, targetName)
        return false
    end

    -- =========================================================
    -- 10. IMPROVEMENT #7: SMART QUEUE SYSTEM
    -- =========================================================
    local EventQueue = {
        queue = {},
        lastCheck = {}
    ...
    function EventQueue:Init(events)
        ...
        for _, event in ipairs(events) do
            table.insert(self.queue, {
                name = event,
                checkCooldown = 25, -- Default cooldown
                lastSeen = 0
            })
        end
    end

    function EventQueue:GetNext()
        local now = tick()
        local best = nil
        ...
        for _, event in ipairs(self.queue) do
            ...
            -- Semakin lama tidak dicek, semakin tinggi prioritasnya
            local score = timeSinceCheck
    ...
    function EventQueue:MarkFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.checkCooldown = 10 -- Check lagi lebih cepat
                event.lastSeen = tick()
                break
            end
        end
    ...
    function EventQueue:MarkNotFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.checkCooldown = 30 -- Jangan check lagi dalam 30 detik
                break
            end
        end
    end
    ...
    -- =========================================================
    -- 12. MAIN TOGGLE (V5 Framework + All Improvements)
    -- =========================================================
    televent:Toggle({
        Title = "Enable Smart Event System",
        Desc = "V5 Framework + All Improvements",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart System", 
                    Content = "Started with improvements", 
                    Duration = 3, 
                    Icon = "cpu" 
                })
                
                print("\n=== V5 IMPROVED PROBE SYSTEM STARTED ===")
                        ...
                        local success, err = pcall(function()
                            ...
                            if SelectedPriorityEvent and SelectedPriorityEvent ~= "" then
                                if ProbeAndFindEvent(SelectedPriorityEvent) then
                                    ...
                                    task.wait(4) -- Hold at priority event
                                end
                            end
                            ...
                            if not foundActive then
                                ...
                                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                                    ...
                                    if hrp then
                                        ...
                                        if dist > 50 then 
                                            local success = TeleportManager:Teleport(data.Pos)
                                            if success then
                                                print("💤 Idle at:", Loch_Return_SelectedArea)
                                            end
                                        end
                                    end
                                end
    ...
    -- =========================================================
    -- 13. PERFORMANCE STATS DISPLAY
    -- =========================================================
    televent:Divider()
    local statsLabel = televent:Label({Text = "System not started..."})
    ...
    local errorLabel = televent:Label({Text = "No errors"})
    task.spawn(function()
        while true do
    ...
    print("✅ [V5 IMPROVED] All improvements loaded successfully!")
end
