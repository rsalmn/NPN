...
local KaizoHubConfig = Window.ConfigManager:CreateConfig("Kaizohub")
...
local function Reg(id, element)
    KaizoHubConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "KaizoHub") .. "/config/"
...
do
    ...
    _G.KaizoHub_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.KaizoHub_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[KaizoHub] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
...
local FishingAreas = {
        ...
        ["Christmas Island"] = {Pos = Vector3.new(708.170, 16.080, 1567.350), Look = Vector3.new(-0.500, 0.000, -0.866)}
    }
    local AreaNames = {}
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_KaizoHub")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_KaizoHub"
    ...
    IconImage.Name = "Icon"
    IconImage.Image = "rbxassetid://18898712828"
...
WindUI:Notify({ Title = "KaizoHub Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    task.wait(5) -- Tunggu UI load sempurna
    
    -- Ganti Load biasa dengan Smart Load
    -- Default load "Kaizohub", atau nama config terakhir user kalau kamu simpan
    SmartLoadConfig("Kaizohub") 
    
    -- Auto Save Loop (Setiap 30 detik) -- Save tetap pakai cara biasa gapapa
    while true do
         task.wait(30)
         pcall(function() KaizoHubConfig:Save() end)
    end
end)
InitializeIcon()
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "KaizoHubESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    _G.KaizoHub_BlatantActive = false
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.KaizoHub_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.KaizoHub_BlatantActive then return false, "Blocked by KaizoHub" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.KaizoHub_BlatantActive and not checkcaller() then
    ...
    local function SuppressGameVisuals(active)
        ...
        if active then
            task.spawn(function()
                ...
                while _G.KaizoHub_BlatantActive do
    ...
    local togblat = Reg("blatantt",blatant:Toggle({
        ...
        Callback = function(state)
            ...
            _G.KaizoHub_BlatantActive = state
...
-- AUTO SELL SECTION
-- =======================================================
local autosell = automatic:Section({
    Title = "Auto Sell",
    TextSize = 20
})
...
local sliderSell = autosell:Slider({
    ...
    Callback = function(v)
        sellDelay = tonumber(v) or 30
    end
})

local togSell = autosell:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            WindUI:Notify({
                Title = "Auto Sell ON",
                Content = "Menjual ikan otomatis tiap " .. sellDelay .. " detik.",
                Duration = 3,
                Icon = "dollar-sign"
            })
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.KaizoHub_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("KaizoHubKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "KaizoHubKaitunStats"
        ...
        txt("KAITUN Kaizo (PREMIUM)", 0.2, Color3.fromRGB(255,0,255), 35)
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "KaizoHub_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_KaizoHub" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "Kaizo Notify" 
    ...
    local GLOBAL_WEBHOOK_URL = "https://discord.com/api/webhooks/1448704629922599033/4sI-MC7xVLc570TCZ1OFgghKYYyRKpBuB-vvi3SA_mIFOVKpEQ72anlEvCBPynA1hf9r"
    local GLOBAL_WEBHOOK_USERNAME = "KaizoHub | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> KaizoHub | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("KaizoHub Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> KaizoHub | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("KaizoHub Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[KaizoHub Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "Kaizohub" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "KaizoHub") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            -- 1. Save ke config utama dulu ("Kaizohub.json")
            KaizoHubConfig:Save()
            task.wait(0.1)

            -- 2. Jika nama beda, salin isi "Kaizohub.json" ke "NamaBaru.json"
            if SelectedConfigName ~= "Kaizohub" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "Kaizohub.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "Kaizohub" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("Kaizohub")
                SelectedConfigName = "Kaizohub"
            else
    ...
    --ConfigSection:Paragraph({
       ...
       -- Desc = "Script otomatis menyimpan ke 'Kaizohub.json' setiap 30 detik.",
      --  Icon = "info"
 --   })
end
...
do
    ...
    about:Section({
        Title = "Join Discord Server KaizoHub",
        TextSize = 20,
    })

    about:Paragraph({
        Title = "KaizoHub Community",
        ...
        Buttons = {
            {
                ...
                Icon = "link",
                Callback = function()
                    setclipboard("https://discord.gg/FMKapD54")
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord KaizoHub berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
