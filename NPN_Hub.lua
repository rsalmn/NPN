...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (LOCHNESS TIMING FIXED)", TextSize = 20 })
    ...
    -- =========================================================
    -- 🕒 LOCHNESS TIMING SYSTEM (FROM PREVIOUS CODE)
    -- =========================================================
    local LochnessSystem = {
        startTime = 1734897600, -- Fixed start time
        interval = 4 * 60 * 60, -- 4 hours in seconds
        activeWindow = 15 * 60, -- 15 minutes active window
        lastCheck = 0,
    ...
    function ActiveEventsCache:IsEventStillActive(eventName)
        ...
        -- Special handling for Lochness
        if eventName == "Lochness Hunt" then
            local isActive = LochnessSystem:IsActive()
            if not isActive then
                print("⚠️ [CACHE] Lochness event window closed")
                self:Remove(eventName)
                return false
            end
        end
        
        local success, isAlive = pcall(function()
            if not cached.model or not cached.model.Parent then
                return false
            end
            return IsEventAlive(cached.model)
        end)
        
        if not success or not isAlive then
            print("⚠️ [CACHE] Event", eventName, "is no longer active")
            self:Remove(eventName)
            return false
        end
    ...
    local function IsEventAlive(obj)
        ...
        local hasParent = pcall(function() 
            return obj.Parent ~= nil 
        end)
        
        if not hasParent then return false end
        ...
        if obj:IsA("Model") then
            ...
            local healthSuccess, healthValue = pcall(function()
                return obj:FindFirstChild("Health")
            end)
            
            if healthSuccess and healthValue and healthValue:IsA("NumberValue") then
                local valueSuccess, value = pcall(function()
                    return healthValue.Value
                end)
                
                if valueSuccess and value and value > 0 then
                    return true
                end
            end
            
            local nameSuccess, modelName = pcall(function()
                return obj.Name
            end)
            
            if nameSuccess and modelName then
                local nameLower = modelName:lower()
                if nameLower:find("hunt") or nameLower:find("event") or 
                   nameLower:find("mega") or nameLower:find("shark") or
                   nameLower:find("worm") or nameLower:find("ghost") or
                   nameLower:find("nessie") or nameLower:find("loch") then
                    return true
                end
            end
        end
        ...
        if obj:IsA("BasePart") then
            ...
            if parentSuccess and parent and parent:IsA("Model") then
                return IsEventAlive(parent)
            end
            
            local nameSuccess, partName = pcall(function()
                return obj.Name
            end)
            
            if nameSuccess and partName then
                local nameLower = partName:lower()
                if nameLower:find("rotation") or nameLower:find("point") or
                   nameLower:find("hunt") or nameLower:find("event") then
                    return true
                end
            end
        end
    ...
    -- =========================================================
    -- 🕒 FIXED PROBE FUNCTION - PROPER LOCHNESS TIMING
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        ...
        -- =========================================================
        -- 🕒 LOCHNESS TIMING CHECK
        -- =========================================================
        if targetName == "Lochness Hunt" then
            -- Check if Lochness is in active window
            ...
            if not isLochActive then
                ...
                return false, nil, nil -- Don't scan if not active
            end
                ...
                if scanSuccess and parts then
                    ...
                    for _, part in ipairs(parts) do
                        ...
                        if checkSuccess and eventFound then
                            ...
                            print("    Position:", eventFound.position)
                            print("    Model:", eventFound.model and eventFound.model.Name or "Unknown")
    ...
    -- =========================================================
    -- 🎯 SMART EVENT FILTER - ONLY SCAN AVAILABLE EVENTS
    -- =========================================================
    local function GetAvailableEvents(eventsToCheck)
        ...
        for _, eventInfo in ipairs(eventsToCheck) do
            ...
            if eventName == "Lochness Hunt" then
                ...
                -- For other events, assume they're always potentially available
    ...
    televent:Toggle({
        Title = "🕒 Smart Event (LOCHNESS TIMING FIXED)",
        Desc = "Only scans Lochness during active window",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with Lochness timing fix", 
                    Duration = 4 
                })
                
                print("\n🕒 === SMART EVENT SYSTEM (LOCHNESS TIMING FIXED) STARTED ===")
                ...
                SmartEventThread = task.spawn(function()
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            -- =========================================================
                            -- PHASE 2: SCAN FOR NEW EVENTS (if needed)
                            -- =========================================================
                            ...
                            -- =========================================================
                            -- PHASE 3: FULL SCAN (with availability filter)
                            -- =========================================================
    ...
    task.spawn(function()
        while true do
            if SmartEventState then
                ...
                for name, _ in pairs(ActiveEventsCache:GetAll()) do
                    cachedCount = cachedCount + 1
                    table.insert(cachedNames, name)
                end
    ...
    print("🕒 [SMART EVENT] Loaded with proper Lochness timing!")
end
