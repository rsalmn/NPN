...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Simple Event System (STAY MODE)", TextSize = 20 })

    -- =========================================================
    -- [1] DATA & CONFIG
    -- =========================================================
    local FishingAreas = {
        ["Ancient Jungle"] = Vector3.new(1535, 3, -193),
        ["Coral Reef"] = Vector3.new(-3207, 6, 2011),
        ["Snowcap"] = Vector3.new(2624, 45, 2623),
        ["Sunstone"] = Vector3.new(-933, 41, -1124),
        ["Terrapin"] = Vector3.new(-167, 7, 196),
        ["Roslit"] = Vector3.new(-1484, 10, 646),
        ["Moosewood"] = Vector3.new(386, 12, 281),
...
do
    ------------------------------------------------------------
    -- 🧠 PRO AI MODE — INTEGRATED WITH STAY MODE SYSTEM
    ------------------------------------------------------------
    
    -- ===== CONFIG =====
    local PRO_AI_MODE = false
    local AI_THREAD = nil
    local AI_SCAN_DELAY = 8 -- Detik antar scan
    local AI_SWITCH_COOLDOWN = 30 -- Cooldown switch event (detik)
    ...
    -- ===== MEMORY SYSTEM =====
    local EventMemory = {}
    local MAX_MEMORY_AGE = 1800 -- 30 menit (detik)

    -- Update memory saat event berhasil/gagal
    local function TouchMemory(event, success)
        ...
        EventMemory[event].totalAttempts = EventMemory[event].totalAttempts + 1
        
        if success then
            EventMemory[event].good = EventMemory[event].good + 1
            EventMemory[event].fail = 0 -- Reset fail streak
        else
            EventMemory[event].fail = EventMemory[event].fail + 1
        end
    end

    -- Cleanup memory lama
    local function CleanOldMemory()
        local now = os.time()
        for event, data in pairs(EventMemory) do
            if now - data.last > MAX_MEMORY_AGE then
                EventMemory[event] = nil
            end
        end
    end

    -- Hitung score event (dynamic)
    local function GetScore(event)
        local base = EventValue[event] or 1
        local mem = EventMemory[event]

        if mem then
            -- Penalty untuk fail streak
            base = base - (mem.fail * 3)
            
            -- Bonus untuk success streak
            base = base + math.min(mem.good * 2, 15)

            -- Heavy penalty jika baru gagal berkali-kali
            if os.time() - mem.last < 20 and mem.fail >= 3 then
                base = base - 25
            end

            -- Bonus jika success rate tinggi
            if mem.totalAttempts > 5 then
                local successRate = mem.good / mem.totalAttempts
    ...
    ------------------------------------------------------------
    -- 🔍 CEK EVENT SPAWNED (IMPROVED)
    ------------------------------------------------------------
    local function IsEventSpawned(name)
        -- Method 1: Check menu rings (original)
        local menu = workspace:FindFirstChild("!!! MENU RINGS")
        if menu then
            for _, ring in ipairs(menu:GetChildren()) do
                if ring:FindFirstChild(name) then
                    return true
                end
            end
        end

        -- Method 2: Check EventCoords existence (fallback)
        if EventCoords[name] then
            local coords = EventCoords[name]
            for _, pos in ipairs(coords) do
                ...
                if result and result.Instance then
                    -- Cek apakah ada model event di sekitar
                    local keywords = EventKeywords[name]
                    if keywords then
                        for _, key in ipairs(keywords) do
                            if string.find(string.lower(result.Instance.Name), string.lower(key)) then
                                return true
                            end
                        end
                    end
        ...
        -- Method 3: Check Lochness special
        if name == "Lochness Hunt" then
            local _, _, active = getLochNextTimes()
            if active then
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj.Name == "Nessie" or obj.Name == "Lochness" then
                        return true
                    end
                end
            end
    ...
    ------------------------------------------------------------
    -- 🎮 INTEGRATION WITH STAY MODE SYSTEM
    ------------------------------------------------------------
    
    -- Start event farming (integrates with existing STAY MODE)
    local function StartAIEvent(eventName)
        if not eventName then return false end

        -- Stop existing system gracefully
        if SystemActive then
            ...
            task.wait(1) -- Delay untuk cleanup
        end

        -- Set new priority
        SelectedPriority = eventName
        SelectedNormal = {} -- Clear normal events saat AI aktif

        -- Restart system
        SystemActive = true
        
        -- Trigger STAY MODE dengan priority baru
        SystemThread = task.spawn(function()
            while SystemActive and PRO_AI_MODE do
                local success = ProcessEvent(eventName)
                
                if success then
                    TouchMemory(eventName, true)
                else
                    TouchMemory(eventName, false)
                    -- Jika gagal, break agar AI bisa pilih event lain
                    break
                end
    ...
    ------------------------------------------------------------
    -- 🧠 AI CONTROLLER LOOP (ENHANCED)
    ------------------------------------------------------------
    local function Start_AI_Controller()
        if AI_THREAD then 
            pcall(function() task.cancel(AI_THREAD) end)
        end
        ...
        AI_THREAD = task.spawn(function()
            ...
            while PRO_AI_MODE do
                ...
                -- Scan semua event yang sedang spawn
                local candidates = {}
                ...
                -- Jika tidak ada event spawned
                if #candidates == 0 then
                    noEventCounter = noEventCounter + 1
                    
                    if noEventCounter >= 3 then -- Setelah 3x scan gagal
                        StopAIEvent()
                        
                        -- Idle mode: TP ke fishing area
                        if IdleArea and FishingAreas[IdleArea] then
                            TP(FishingAreas[IdleArea])
                        end
                        ...
                        WindUI:Notify({
                            Title = "🔍 AI Scanning",
                            Content = "Tidak ada event spawned, idle mode...",
                            Duration = 3,
                            Icon = "search"
                        })
                ...
                local bestEvent = candidates[1]

                -- Debug: Log semua scores
                local scoreText = ""
                for i, c in ipairs(candidates) do
                    scoreText = scoreText .. string.format("%s: %d | ", c.name, c.score)
                end
                print("[AI DEBUG] Scores:", scoreText)

                -- Decision: Switch jika:
                -- 1. Event berbeda
                -- 2. Score jauh lebih tinggi (> 20 poin)
                -- 3. Cooldown sudah lewat
                local shouldSwitch = false
                local currentEvent = SelectedPriority

                if bestEvent.name ~= currentEvent then
                    local now = os.time()
                    
                    -- Cek cooldown
                    if now - lastSwitchTime >= AI_SWITCH_COOLDOWN then
                        -- Cek score difference
                        local currentScore = GetScore(currentEvent or "")
                        local scoreDiff = bestEvent.score - currentScore
                        
                        if scoreDiff > 20 or not currentEvent then
                            shouldSwitch = true
                        end
                    end
                end
                ...
                if shouldSwitch then
                    ...
                    local success = StartAIEvent(bestEvent.name)
                    
                    if success then
                        lastSwitchTime = os.time()
            ...
            -- Cleanup saat AI dimatikan
            StopAIEvent()
            WindUI:Notify({
                Title = "🧠 PRO AI MODE",
                Content = "AI Disabled - Return to manual mode",
                Duration = 3,
                Icon = "power"
            })
        end)
    ...
    ------------------------------------------------------------
    farm:Divider()
    local aiSection = farm:Section({ 
        Title = "🧠 Smart Event AI System (BETA)", 
        TextSize = 20 
    })
    ...
    aiSection:Paragraph({
        Title = "How It Works",
        Content = [[
• AI akan scan event yang spawned setiap 8 detik
• Memilih event terbaik berdasarkan value + history
• Auto-switch jika ada event lebih menguntungkan
• Integrasi penuh dengan STAY MODE
• Memory system: belajar dari success/fail
        ]]
    })

    -- Config: Scan delay
    Reg("AIScanDelay", aiSection:Slider({
        Title = "Scan Interval (seconds)",
        Description = "Seberapa sering AI cek event baru",
        Min = 5,
        Max = 30,
        Default = 8,
        Callback = function(value)
    ...
    -- Config: Switch cooldown
    Reg("AISwitchCooldown", aiSection:Slider({
        Title = "Switch Cooldown (seconds)",
        Description = "Min. jeda antar switch event",
        Min = 15,
        Max = 60,
        Default = 30,
        Callback = function(value)
            AI_SWITCH_COOLDOWN = value
        end
    }))

    -- Main toggle
    Reg("ProAIToggle", aiSection:Toggle({
        Title = "Enable PRO AI Mode",
        Description = "AI akan take over event selection",
        Default = false,
        Callback = function(state)
            PRO_AI_MODE = state
            
            if state then
                -- Disable STAY MODE manual toggle jika ada
                if SystemActive then
                    WindUI:Notify({
                        Title = "⚠️ Warning",
                        Content = "Manual STAY MODE akan diambil alih AI",
                        Duration = 4,
                        Icon = "alert-triangle"
                    })
                end
    ...
    aiSection:Button({
        ...
        Description = "Clear semua event history",
        Callback = function()
            EventMemory = {}
            WindUI:Notify({
                Title = "✅ Memory Cleared",
                Content = "AI akan mulai fresh learning",
                Duration = 3,
                Icon = "trash-2"
            })
        end
    ...
    aiSection:Button({
        Title = "📊 Show AI Stats",
        Callback = function()
            local stats = "=== AI MEMORY ===\n"
            local hasData = false
            
            for event, data in pairs(EventMemory) do
                hasData = true
                stats = stats .. string.format(
                    "%s: Good=%d | Fail=%d | Score=%d\n",
                    event,
                    data.good,
                    data.fail,
                    GetScore(event)
    ...
    -- Visual indicator
    local aiStatusLabel = Reg("AIStatusLabel", aiSection:Paragraph({
        Title = "AI Status",
        Content = "❌ AI Inactive"
    }))
    ...
    task.spawn(function()
        while true do
            ...
            if PRO_AI_MODE then
                local current = SelectedPriority or "None"
                local score = GetScore(current)
