...
do
    ...
    local RotationSystem = {
        ...
        priorityBonus = 0.5,        -- priority events get visited more often
        lastRotation = 0,
        currentEventIndex = 0,
        rotationQueue = {},
        stayDuration = 8,           -- seconds to stay at each event
        lastEventChange = 0
    }
    ...
    function RotationSystem:BuildQueue()
        ...
        if SelectedPriorityEvent and SelectedPriorityEvent ~= "" then
            for eventName, data in pairs(ActiveEventsCache:GetAll()) do
                if eventName == SelectedPriorityEvent and ActiveEventsCache:IsEventStillActive(eventName) then
        ...
        -- Shuffle non-priority events for variety
        if #self.rotationQueue > 2 then
            ...
            for _, event in ipairs(self.rotationQueue) do
                if event.priority then
                    ...
                    table.insert(normalEvents, event)
                end
            end
            
            -- Shuffle normal events
            for i = #normalEvents, 2, -1 do
                local j = math.random(i)
                normalEvents[i], normalEvents[j] = normalEvents[j], normalEvents[i]
            end
            
            -- Rebuild queue with mixed priority and normal events
            self.rotationQueue = {}
            local normalIndex = 1
            local priorityIndex = 1
            
            while normalIndex <= #normalEvents or priorityIndex <= #priorityEvents do
                -- Add priority event every 2-3 events
                if priorityIndex <= #priorityEvents and (#self.rotationQueue % 3 == 0 or normalIndex > #normalEvents) then
                    table.insert(self.rotationQueue, priorityEvents[priorityIndex])
                    priorityIndex = priorityIndex + 1
                end
                
                -- Add normal event
                if normalIndex <= #normalEvents then
                    table.insert(self.rotationQueue, normalEvents[normalIndex])
    ...
    function RotationSystem:ShouldRotate()
        local now = tick()
        return now - self.lastRotation >= self.interval
    end
    ...
    function RotationSystem:SetInterval(seconds)
        self.interval = math.max(5, math.min(60, seconds)) -- Between 5-60 seconds
        print("🔄 [ROTATION] Interval set to", self.interval, "seconds")
    end

    -- =========================================================
    -- LOCHNESS TIMING SYSTEM (same as before)
    -- =========================================================
    ...
    -- =========================================================
    -- EVENT COORDINATES & KEYWORDS (same as before)
    -- =========================================================
    local EventCoords = {
        ["Shark Hunt"] = {
    ...
    -- =========================================================
    -- WORM PARTICLE DETECTION (same as before)
    -- =========================================================
    ...
    -- =========================================================
    -- ACTIVE EVENTS CACHE WITH ROTATION SUPPORT
    -- =========================================================
    ...
    function ActiveEventsCache:Add(eventName, position, model)
        ...
        -- Rebuild rotation queue when new event is added
    ...
    function ActiveEventsCache:Remove(eventName)
        if self.events[eventName] then
            ...
            -- Rebuild rotation queue when event is removed
            RotationSystem:BuildQueue()
        end
    end
    ...
    -- =========================================================
    -- TELEPORT MANAGER (same as before)
    -- =========================================================
    ...
    -- =========================================================
    -- PROBE FUNCTION (same as before but shorter for space)
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        ...
        if targetName == "Lochness Hunt" then
            ...
            if not isLochActive then return false, nil, nil end
    ...
    -- =========================================================
    -- UI SETUP WITH ROTATION CONTROLS
    -- =========================================================
    televent:Dropdown({
...
televent:Input({
    ...
    Desc = "Seconds between event switches",
    Default = tostring(RotationSystem.interval), -- Shows current value
    ...
    televent:Toggle({
        ...
        Desc = "Auto-rotate between all found events every " .. RotationSystem.interval .. "s",
        Value = false,
        Callback = function(state)
                    ...
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            if hasActiveEvents then
                                print("\n🔄 [ROTATION MODE] Managing active events...")
                                
                                -- Check if it's time to rotate
                                if RotationSystem:ShouldRotate() then
                                    local nextEvent = RotationSystem:GetNextEvent()
                                    
                                    if nextEvent then
                                        print("📍 [ROTATION] Switching to:", nextEvent.name)
                                        print("   Position:", nextEvent.data.position)
                                        print("   Priority:", nextEvent.priority and "YES" or "NO")
                                        print("   Visit Count:", nextEvent.data.visitCount or 0)
                            ...
                            -- No active events, scan for new ones
                            ...
                            for _, eventInfo in ipairs(availableEvents) do
                                ...
                                if found and position and model then
                                    ...
                                    -- Start rotation immediately after finding first event
                                    RotationSystem.lastRotation = 0
                                end
    ...
    print("🔄 [SMART EVENT] Loaded with rotation system!")
end
