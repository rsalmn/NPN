...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (ERROR FIXED)", TextSize = 20 })
    ...
    local EventCoords = {
        ...
        ["Megalodon Hunt"] = {
            ...
            Vector3.new(-1191, -1.3, 3597), -- This one worked in your log!
    ...
    -- =========================================================
    -- FIXED IsEventAlive - This was causing the nil error!
    -- =========================================================
    local function IsEventAlive(obj)
        ...
        if obj:IsA("Model") then
            ...
            if success and humanoid then
                ...
                if healthSuccess and health and health > 0 then
                    print("    ✅ Model alive - Humanoid HP:", health)
                    return true
                end
            end
            
            -- Check Health value with pcall
            ...
            if healthSuccess and healthValue and healthValue:IsA("NumberValue") then
                ...
                if valueSuccess and value and value > 0 then
                    print("    ✅ Model alive - Health value:", value)
                    return true
                end
            end
            
            -- For event models that might not have health indicators
            -- If model exists and has parent, consider it "alive"
            ...
            if nameSuccess and modelName then
                ...
                if nameLower:find("hunt") or nameLower:find("event") or 
                   ...
                   nameLower:find("worm") or nameLower:find("ghost") then
                    print("    ✅ Event model detected (assuming alive):", modelName)
                    return true
                end
            end
        end
        
        -- For Parts - check if parent is alive
        if obj:IsA("BasePart") then
            ...
            -- If it's just a part with event-related name, consider it alive
            ...
            if nameSuccess and partName then
                ...
                if nameLower:find("rotation") or nameLower:find("point") or
                   nameLower:find("hunt") or nameLower:find("event") then
                    print("    ✅ Event part detected:", partName)
                    return true
                end
            end
        end
    ...
    local function ProbeAndFindEvent(targetName)
        ...
        for i, pos in ipairs(coords) do
            ...
            -- Brief wait for chunk load
            task.wait(1.5)
            
            -- Scan with multiple radii
                    ...
                    for _, part in ipairs(parts) do
                        local checkSuccess, checkResult = pcall(function()
                            ...
                            if partMatch or parentMatch then
                                ...
                                print("    ✓ Matched keyword:", matchedKeyword, "in", parentName ~= "" and parentName or partName)
                                ...
                                if model then
                                    if IsEventAlive(model) then
                                        print("    ✅✅✅ [EVENT CONFIRMED]", model.Name)
                                        return true
                                    else
                                        print("    ⚠️ Model found but not alive:", model.Name)
                                    end
    ...
    function ErrorHandler:Handle(err)
        ...
        if self.ErrorCount >= self.MaxErrors then
            ...
            warn("🛑 Max errors reached - stopping system")
            return false
        end
        return true
    end
    ...
    televent:Toggle({
        Title = "✅ Smart Event (ERROR FIXED)",
        Desc = "Fixed nil errors + improved detection",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with error protection", 
                    Duration = 4 
                })
                
                print("\n✅ === SMART EVENT SYSTEM STARTED (ERROR FIXED) ===")
                    ...
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            if SelectedPriorityEvent and SelectedPriorityEvent ~= "" then
                                local probeSuccess, probeResult = pcall(ProbeAndFindEvent, SelectedPriorityEvent)
                                
                                if probeSuccess and probeResult then
                                    foundActive = true
                            ...
                            if not foundActive and #SelectedNormalEvents > 0 then
                                ...
                                if nextEvent then
                                    local probeSuccess, probeResult = pcall(ProbeAndFindEvent, nextEvent.name)
                            ...
                            if not foundActive then
                                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                                    ...
                                    local idleSuccess = pcall(function()
                                        local char = Players.LocalPlayer.Character
                                        local hrp = char and char:FindFirstChild("HumanoidRootPart")
    ...
    task.spawn(function()
        while true do
            if SmartEventState then
                local lastError = ErrorHandler.LastError and string.sub(ErrorHandler.LastError, 1, 30) or "None"
                statsText:Set({
                    Title = "✅ Smart Event System",
                    Desc = string.format("🎯 Active | Errors: %d/%d | TPs: %d\nLast: %s", 
                        ErrorHandler.ErrorCount,
                        ErrorHandler.MaxErrors,
    ...
    print("✅ [SMART EVENT] Loaded with error protection!")
end
