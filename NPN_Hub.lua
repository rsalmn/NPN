...
do
    ...
    -- Cek keberadaan target monster berdasarkan nama (radius 800)
                ...
                if primary then
                    ...
                    if dist < 800 then
                        for _, key in ipairs(keywords) do
                            if string.find(string.lower(obj.Name), string.lower(key)) 
                                and not Players:GetPlayerFromCharacter(obj) then
                                if dist > 50 then TP(primary.Position) end
                                return true
                            end
                        end
                    end
    ...
    -- Logic utama untuk memproses 1 event tertentu
    local function ProcessEvent(eventName)
        if not eventName or not SystemActive then return false end
            ...
            if active then
                for _, v in ipairs(workspace:GetDescendants()) do
                    if v.Name == "Nessie" or v.Name == "Lochness" then
                        local pivot = nil
                        pcall(function() pivot = v:GetPivot().Position end)
                        if pivot then TP(pivot) end
                        WindUI:Notify({Title="STAY", Content="Farming Lochness...", Duration=3})
                        repeat 
                            task.wait(1)
                        until not v.Parent or not SystemActive
                        return true
                    end
                end
                -- tidak ketemu fisiknya → tunggu di posisi aman
    ...
    televent:Toggle({
        ...
        Desc = "Logic: Temu → Diam Sampai Habis → Cek Lagi",
        Value = false,
        Callback = function(state)
            SystemActive = state
            if state then
                ...
                SystemThread = task.spawn(function()
                    while SystemActive do
                        ...
                        -- 2. Cek event biasa (jika tidak sedang farming)
                        ...
                        -- 3. Jika tidak ada event aktif → idle
                        if not farming then
                            if IdleArea and FishingAreas[IdleArea] then
                                local p = FishingAreas[IdleArea]
                                local c = LocalPlayer.Character
                                if c and c:FindFirstChild("HumanoidRootPart") then
                                    if (c.HumanoidRootPart.Position - p).Magnitude > 100 then
                                        TP(p)
                                    end
                                end
                            end
