...
do
    ...
    -- FIX #1: Improved Keywords dengan case variations
    local EventKeywords = {
        ["Megalodon Hunt"] = {"megalodon", "mega", "giant", "boss"},
        ["Shark Hunt"] = {"shark", "great white", "predator", "jaws"}, 
        ["Ghost Shark Hunt"] = {"ghost", "phantom", "spectral", "spirit"},
        ["Worm Hunt"] = {"worm", "sandworm", "giant worm", "desert"},
        ["Ghost Worm"] = {"ghost", "phantom", "spirit", "worm"},
        ["Treasure Event"] = {"chest", "crate", "supply", "treasure", "loot"},
        ["Meteor Rain"] = {"meteor", "asteroid", "space", "rock"},
    }

    -- =========================================================
    -- 3. TELEPORT MANAGER (Rate Limiting)
    -- =========================================================
    ...
    function TeleportManager:Teleport(pos)
        ...
        if not canTP then
            print("⚠️ Teleport blocked:", reason)
            return false
        end
        ...
        if self.LastPosition and (finalPos - self.LastPosition).Magnitude < 35 then
            return false
        end
    ...
    local function WaitForChunkLoad(position, timeout)
        ...
        while tick() - startTime < timeout do
            -- Check if area has reasonable amount of parts
            ...
            local success, parts = pcall(function()
                return workspace:GetPartBoundsInBox(
                    CFrame.new(position), 
                    Vector3.new(120, 50, 120), 
                    params
                )
            end)
            
            if success and #parts > 10 then -- Reasonable part count
                return true
            end
    ...
    -- FIX #2: Enhanced Event Detection
    local function IsEventAlive(obj)
        if not obj then return false end
        
        -- For models with humanoids
        if obj:IsA("Model") then
            ...
            -- Check for health values
            local health = obj:FindFirstChild("Health")
            if health and health:IsA("NumberValue") and health.Value > 0 then
                return true
            end
            
            -- Check for specific event parts
            if obj:FindFirstChild("Treasure") or obj:FindFirstChild("Chest") or obj:FindFirstChild("Crate") then
                return true
            end
        end
        
        -- For parts that might be events themselves
    ...
    -- FIX #3: Better Scanning Algorithm
    local function EnhancedScan(position, keywords, targetName)
        print("🔍 Enhanced scanning at:", position)
        
        -- Multiple search strategies
        local strategies = {
            -- Strategy 1: GetPartBoundsInBox (fast, limited range)
            function()
                local params = OverlapParams.new()
                params.FilterDescendantsInstances = {Players.LocalPlayer.Character}
                params.FilterType = Enum.RaycastFilterType.Exclude
                ...
                for _, radius in ipairs({150, 300, 500}) do
                    local success, parts = pcall(function()
                        return workspace:GetPartBoundsInBox(
                            CFrame.new(position), 
                            Vector3.new(radius, 300, radius), 
                            params
                        )
                    end)
                    
                    if success then
                        print("📦 Found", #parts, "parts in radius", radius)
                        ...
                        for _, part in ipairs(parts) do
                            ...
                            for _, keyword in ipairs(keywords) do
                                if partName:find(keyword) or parentName:find(keyword) then
                                    local model = part:FindFirstAncestorOfClass("Model") or part.Parent
                                    if IsEventAlive(model or part) then
                                        print("✅ Found event via PartBounds:", model and model.Name or part.Name)
            ...
            -- Strategy 2: Workspace children scan (broader)
            function()
                for _, obj in ipairs(workspace:GetChildren()) do
                    if obj:IsA("Model") and obj ~= Players.LocalPlayer.Character then
                        local objName = obj.Name:lower()
                        local distance = obj.PrimaryPart and (obj.PrimaryPart.Position - position).Magnitude or math.huge
                        
                        if distance <= 800 then -- Within reasonable range
                            for _, keyword in ipairs(keywords) do
                                if objName:find(keyword) then
                                    if IsEventAlive(obj) then
                                        print("✅ Found event via Workspace:", obj.Name)
                                        return true, obj.PrimaryPart and obj.PrimaryPart.Position or position
                                    end
                                end
                            end
                        end
            ...
            -- Strategy 3: CollectionService tags
            function()
                local tagged = {}
                pcall(function()
                    tagged = CollectionService:GetTagged("Boss")
                end)
                ...
                for _, obj in ipairs(tagged) do
                    if obj.Parent == workspace then
                        ...
                        for _, keyword in ipairs(keywords) do
                            if objName:find(keyword) then
                                if IsEventAlive(obj) then
                                    print("✅ Found event via Tags:", obj.Name)
                                    return true, obj:IsA("Model") and obj.PrimaryPart and obj.PrimaryPart.Position or obj.Position
                                end
                            end
                        end
                    end
        ...
        for i, strategy in ipairs(strategies) do
            print("🔍 Strategy", i, "for", targetName)
            local found, pos = strategy()
            if found then
                return true, pos
            end
        end
    ...
    function ErrorHandler:Handle(err)
        ...
        print("⚠️ Error #" .. self.ErrorCount .. ":", err)
    ...
    function LochnessDetector:Find()
        local now = tick()
        if now - self.LastScan < self.ScanCooldown then
            return nil
        end
        ...
        if not active then
            print("⏳ Lochness window not active")
            return nil
        end
    ...
    -- =========================================================
    -- 7. MAIN PROBE FUNCTION
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        if not targetName then return false end
        
        print("🕵️ [PROBE START] " .. targetName)
        
        -- LOCHNESS SPECIAL CASE
        if targetName == "Lochness Hunt" then
            ...
            if pos then
                ...
                print("⏳ [Lochness] Not found or not active")
                -- Go to idle area
        ...
        if not coords then 
            print("⚠️ No coordinates for " .. targetName)
            return false 
        end

        local keywords = EventKeywords[targetName] or {}
        if #keywords == 0 then
            print("⚠️ No keywords for " .. targetName)
            return false
        end
        ...
        for i, pos in ipairs(coords) do
            print("📍 Checking spot #" .. i .. " for " .. targetName)
            ...
            WaitForChunkLoad(pos, 3)
            
            -- Enhanced scanning
            local found, eventPos = EnhancedScan(pos, keywords, targetName)
            
            if found then
                print("✅ [SUCCESS] Found " .. targetName .. " at spot #" .. i)
                
                -- Move closer to event if needed
                if eventPos and (eventPos - pos).Magnitude > 30 then
                    TeleportManager:Teleport(eventPos)
                end
                
                return true
            else
                print("❌ Not found at spot #" .. i)
            end
            
            task.wait(0.5) -- Brief pause between spots
        end

        print("❌ [FAILED] " .. targetName .. " not found at any location")
        return false
    end

    -- =========================================================
    -- 8. SMART QUEUE SYSTEM
    -- =========================================================
    local EventQueue = {
        queue = {},
        lastCheck = {}
    ...
    function EventQueue:Init(events)
        ...
        print("📋 Queue initialized with", #self.queue, "events")
    end
    ...
    function EventQueue:GetNext()
        ...
        -- Find events that are ready to check
        ...
        if #availableEvents == 0 then
            return nil -- No events ready
        end
        
        -- Pick the event that hasn't been checked the longest
        local bestEvent = availableEvents[1]
        local oldestTime = self.lastCheck[bestEvent.name] or 0
        
        for _, event in ipairs(availableEvents) do
            local checkTime = self.lastCheck[event.name] or 0
            if checkTime < oldestTime then
                bestEvent = event
                oldestTime = checkTime
            end
        end
        
        self.lastCheck[bestEvent.name] = now
        print("📋 Next event from queue:", bestEvent.name)
        return bestEvent
    end
    ...
    function EventQueue:MarkFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.checkCooldown = 15 -- Check again sooner
                event.lastFound = tick()
                break
            end
        end
    end
    ...
    function EventQueue:MarkNotFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.checkCooldown = 45 -- Wait longer before checking again
                break
            end
        end
    end
    ...
    televent:Dropdown({
        ...
        Callback = function(opt)
            ...
            print("🌟 Priority event:", opt or "None")
        end
    })
    ...
    televent:Dropdown({
        ...
        Callback = function(opts) 
            ...
            print("⚔️ Selected events:", table.concat(SelectedNormalEvents, ", "))
        end
    })
    ...
    televent:Toggle({
        ...
        Desc = "V5 Enhanced with Better Detection",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart System", 
                    Content = "V5 Enhanced Started", 
                    Duration = 3 
                })
                
                print("\n🚀 === SMART EVENT V5 ENHANCED STARTED ===")
                
                -- Stop old event system
                pcall(function() 
                ...
                SmartEventThread = task.spawn(function()
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            -- PHASE 1: Priority Event
                            if SelectedPriorityEvent and SelectedPriorityEvent ~= "" then
                                print("\n⭐ Checking Priority:", SelectedPriorityEvent)
                                if ProbeAndFindEvent(SelectedPriorityEvent) then
                                    foundActive = true
                                    print("⭐ PRIORITY ACTIVE:", SelectedPriorityEvent)
                                    task.wait(8) -- Stay longer at priority event
                                end
                            end

                            -- PHASE 2: Normal Events Queue
                            if not foundActive and #SelectedNormalEvents > 0 then
                                local nextEvent = EventQueue:GetNext()
                                if nextEvent then
                                    print("\n⚔️ Checking Queue Event:", nextEvent.name)
                                    if ProbeAndFindEvent(nextEvent.name) then
                                        foundActive = true
                                        EventQueue:MarkFound(nextEvent.name)
                                        print("⚔️ QUEUE EVENT ACTIVE:", nextEvent.name)
                                        task.wait(6) -- Farm time
                                    else
                                        EventQueue:MarkNotFound(nextEvent.name)
                                        task.wait(1) -- Short pause after failed check
                                    end
                                else
                                    print("📋 No events ready in queue")
                                    task.wait(2)
                                end
                            end
                            ...
                            if not foundActive then
                                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                                    ...
                                    if hrp then
                                        ...
                                        if dist > 60 then 
                                            print("💤 Returning to idle area:", Loch_Return_SelectedArea)
                                            TeleportManager:Teleport(data.Pos)
                                        end
                                    end
                                end
                                task.wait(3) -- Idle wait
                            end
                        end)
                        ...
                        if not success then
                            ...
                            task.wait(5) -- Error recovery wait
                        else
    ...
    -- =========================================================
    -- 11. STATUS DISPLAY (Using Text/Paragraph instead of Label)
    -- =========================================================
    televent:Divider()
    local statsText = televent:Paragraph({
        Title = "📊 System Status",
        Desc = "System not started..."
    })
        ...
        while true do
            if SmartEventState then
                local status = string.format(
                    "Status: Active | Errors: %d | TP Count: %d | Queue: %d events",
                    ErrorHandler.ErrorCount,
                    TeleportManager.TeleportCount,
                    #SelectedNormalEvents
                )
                statsText:Set({
                    Title = "📊 System Status",
                    Desc = status
    ...
    print("✅ [V5 ENHANCED] Fixed and loaded successfully!")
end
