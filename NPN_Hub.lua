...
do
    ...
    ----------------------------------------------------
    -- PRIORITY & NORMAL EVENT SYSTEM
    ----------------------------------------------------
    local PriorityEvent = nil
    local NormalEvents = {}
    local NormalMap = {}
    ...
    televent:Dropdown({
        ...
        Callback = function(opt)
            PriorityEvent = opt
            if opt then
                SetStatus("Priority event: " .. opt)
            else
                SetStatus("Priority event cleared.")
            end
        end
    })
    ...
    ----------------------------------------------------
    -- UI : NORMAL EVENT MULTI SELECT
    ----------------------------------------------------
    televent:Paragraph({
        Title = "Normal Event List",
        Content = "Event ini akan dijalankan jika PRIORITY tidak ada"
    })

    local function RefreshNormalList()
        NormalEvents = {}
        for ev, enabled in pairs(NormalMap) do
            if enabled then
    ...
    task.spawn(function()
        while true do
            if not autoEventTeleportState then
                task.wait(1)
                continue
            end

            ------------------------------------
            -- 1️⃣ Cek Priority Event dahulu
            ------------------------------------
            if PriorityEvent and PriorityEvent ~= "Lochness Hunt" then
                local ok = EventTP.Start(PriorityEvent)
                if ok then
                    SetStatus("Teleporting PRIORITY: " .. PriorityEvent)
                    
                    -- tunggu scan beberapa saat
                    task.wait(6)
                    goto CONTINUE
                end
            end
            ...
            ------------------------------------
            -- 2️⃣ Kalau tidak ada Priority aktif
            --    cek NORMAL EVENT
            ------------------------------------
            if #NormalEvents > 0 then
                
                -- hanya 1 event → langsung jalankan
                if #NormalEvents == 1 then
                    local ev = NormalEvents[1]
                    EventTP.Start(ev)
                    SetStatus("Teleporting Normal Event: "..ev)
                    task.wait(6)
                    goto CONTINUE
                end

                ------------------------------------
                -- 🔁 Banyak Normal Events aktif
                -- Rotate + Delay 5 detik
                ------------------------------------
                for i, ev in ipairs(NormalEvents) do
                    if not autoEventTeleportState then break end
                    if PriorityEvent then break end

                    EventTP.Start(ev)

                    WindUI:Notify({
                        Title = "Switch Event",
                        Content = "Pindah ke "..ev,
                        Duration = 3,
                        Icon = "refresh-ccw"
                    })
