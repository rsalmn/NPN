...
do
    farm:Divider()
local televent = farm:Section({ Title = "Advanced Event System (STAY MODE)", TextSize = 20 })
...
local AreaNames = {}
for name in pairs(FishingAreas) do table.insert(AreaNames, name) end
...
local EventConfig = {
    ...
    ["Lochness Hunt"] = {
        Coords = {Vector3.new(0, 0, 0)}, -- Placeholder
        Keywords = {"Lochness", "Nessie"},
        DetectionRadius = 300,
        IsSpecial = true
    }
}
...
local function SafeTeleport(position)
    ...
    -- Add Y offset to prevent stuck in ground
...
local function IsMonsterPresent(eventName)
    if not eventName then return false end
    
    local config = EventConfig[eventName]
    ...
    -- Efficient scanning using workspace.GetPartBoundsInRadius
    local monsters = {}
    for _, obj in ipairs(workspace:GetPartBoundsInRadius(
        myPosition, 
...
local function TrackMonster(monster)
    ActiveEvent = monster
    ...
    WindUI:Notify({
        Title = "STAY MODE ACTIVE",
        Content = "Tracking "..monster.Name,
        Duration = 3,
        Icon = "map-pin"
    })
    
    -- Maintain position while monster exists
    while monster.Parent and SystemActive do
        local monsterPos = monster:GetPivot().Position
        local myPos = LocalPlayer.Character.HumanoidRootPart.Position
        
        if (monsterPos - myPos).Magnitude > 10 then
            SafeTeleport(monsterPos)
        end
...
-- =============================================================================
-- [4] MAIN EVENT PROCESSING LOGIC
-- =============================================================================
local function ProcessEvent(eventName)
    if not eventName or SystemActive == false then 
        return false 
    end
    ...
    if eventName == "Lochness Hunt" then
        ...
        if not active then
            WindUI:Notify({
                Title = "Lochness Inactive",
                Content = "Event not available yet",
                Duration = 3,
                Icon = "clock"
            })
            return false
        ...
        -- Search for Lochness directly
        local lochness = workspace:FindFirstChild("Nessie") or workspace:FindFirstChild("Lochness")
        if lochness then
            SafeTeleport(lochness:GetPivot().Position)
            TrackMonster(lochness)
            return true
        end
    end
    
    -- Regular event scanning
    for _, coord in ipairs(config.Coords) do
        if not SystemActive then break end
        
        SafeTeleport(coord)
        task.wait(2)  -- Allow time to load
        
        local found, monsters = IsMonsterPresent(eventName)
        if found then
            TrackMonster(monsters[1])
            return true
        end
    end
...
-- [5] AUTOMATION ENGINE
-- =============================================================================
local function StartEventSystem()
    if SystemThread then 
        task.cancel(SystemThread) 
        SystemThread = nil 
    end
    ...
    SystemThread = task.spawn(function()
        while SystemActive do
            ...
            if SelectedPriority and SelectedPriority ~= "" then
                eventProcessed = ProcessEvent(SelectedPriority)
            end
            
            -- Process normal events
            if not eventProcessed and #SelectedNormal > 0 then
                for _, eventName in ipairs(SelectedNormal) do
                    if not SystemActive then break end
                    
                    eventProcessed = ProcessEvent(eventName)
                    if eventProcessed then break end
                end
            end
            
            -- Idle mode
            if not eventProcessed and IdleArea and FishingAreas[IdleArea] then
                SafeTeleport(FishingAreas[IdleArea])
                WindUI:Notify({
                    Title = "IDLE MODE",
                    Content = "Waiting at "..IdleArea,
                    Duration = 3,
                    Icon = "coffee"
                })
                task.wait(30)  -- Longer wait in idle mode
            else
                task.wait(5)  -- Short wait between cycles
            end
        end
    end)
end
...
-- Normal Events Selector
televent:Dropdown({
    Title = "Normal Events",
    Values = eventsList,
    Multi = true,
    Callback = function(values)
        SelectedNormal = values
        WindUI:Notify({
            Title = "Events Updated",
            Content = #values.." events selected",
            Duration = 2,
            Icon = "list"
        })
    end
...
-- Main Toggle
televent:Toggle({
    Title = "Start Auto Event System",
    Desc = "Priority > Normal > Idle",
    Value = false,
    Callback = function(state)
        SystemActive = state
        
        if state then
            -- Disable other teleport systems
            if EventTP and EventTP.Stop then EventTP.Stop() end
            
            -- Enable essential features
            if WalkOnWaterToggleElement then 
                WalkOnWaterToggleElement:Set(true) 
            end
            
            WindUI:Notify({
                Title = "SYSTEM ACTIVATED",
                Content = "Starting event automation",
                Duration = 3,
                Icon = "play"
            })
            ...
            WindUI:Notify({
                ...
                Content = "Event automation stopped",
                Duration = 3,
                Icon = "stop"
            })
        end
...
televent:Label({
    Title = "System Status",
    Text = function()
        if not SystemActive then return "OFFLINE" end
        if ActiveEvent then return "TRACKING: "..ActiveEvent.Name end
        return "SCANNING..."
    end,
...
local televenttt = farm:Section({ Title = "Smart Event System (FINAL LOCK)", TextSize = 20 })
------------------------------------------------------------
-- ⚙️ WINDUI — SECTION AI MODE
------------------------------------------------------------
local aiSec = televenttt:Section({
    Title = "PRO AI Mode",
    TextSize = 20
})
