...
do
    farm:Divider()
    local televent = farm:Section({
        Title = "Smart Event System (Production Version)",
        TextSize = 20
    })

    ----------------------------------------------------------------
    -- ① KONFIGURASI & VARIABLE DASAR
    ----------------------------------------------------------------
    local Players      = game:GetService("Players")
    local LocalPlayer  = Players.LocalPlayer
    local Workspace    = game:GetService("Workspace")

    local SAFE_LAND_POSITION = Vector3.new(0, 15, 0)
    local WAIT_FARM   = 3         -- waktu diam saat farming
    local WAIT_SCAN   = 1.5       -- waktu menunggu tiap teleport scan
    local WAIT_IDLE   = 3         -- jeda saat idle
    local SCAN_RADIUS = Vector3.new(450, 180, 450) -- ukuran area scan
    ...
    local FishingAreas = {
        ...
        ["Snowcap"]        = {Pos = Vector3.new(2624.000, 45.000, 2623.000), Look = Vector3.new(0,0,-1)},
        ["Tropical Island"]= {Pos = Vector3.new(-2162.920, 2.825, 3638.445), Look = Vector3.new(0.381, 0, 0.925)},
        ["Christmas Island"]= {Pos = Vector3.new(1136.833, 23.573, 1562.916), Look = Vector3.new(0.790, 0, -0.613)},
    }
    local AreaNames = {}
    ...
    local function TeleportLookAt(position, lookVector)
        local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
        local hrp  = char:WaitForChild("HumanoidRootPart",5)
        if hrp then
            hrp.CFrame = CFrame.new(position, position + lookVector)
        end
    end
    ...
    -- ⑥ DETEKSI EVENT
    ----------------------------------------------------------------
    local function CheckLocalPresence(target)
        local char = LocalPlayer.Character
        local hrp  = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return false end

        if target=="Lochness Hunt" then
            local _,_,active = getLochNextTimes()
            if not active then return false end
            for _,obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") and (string.find(obj.Name,"Nessie") or string.find(obj.Name,"Loch")) then
                    if (obj:GetPivot().Position - hrp.Position).Magnitude < 600 then
                        return true
                    end
                end
            end
        ...
        local keywords = EventKeywords[target] or {target}
        local params = OverlapParams.new()
        params.FilterDescendantsInstances = {char, workspace.Terrain}
        params.FilterType = Enum.RaycastFilterType.Exclude
        local parts = workspace:GetPartBoundsInBox(hrp.CFrame, SCAN_RADIUS, params)
        ...
        for _,part in ipairs(parts) do
            ...
            for _,key in ipairs(keywords) do
                if string.find(pname,key) or string.find(mname,key) then
                    ...
                    if model and (model:FindFirstChild("Humanoid") or model:FindFirstChild("Health") or target=="Treasure Event") then
                        return true
                    end
                end
            end
    ...
    local function ProbeAndFindEvent(target)
        ...
        local coords = EventCoords[target]
        if not coords then return false end

        for _,pos in ipairs(coords) do
            SafeTeleportTo(pos)
            task.wait(WAIT_SCAN)
            if CheckLocalPresence(target) then return true end
        end
        return false
    end
    ...
    local function GoIdle()
        if IdleArea and FishingAreas[IdleArea] then
            local data = FishingAreas[IdleArea]
            TeleportLookAt(data.Pos, data.Look)
        else
            SafeTeleportTo(SAFE_LAND_POSITION)
        end
    end
    ...
    televent:Dropdown({
        ...
        Callback=function(opt)
            ...
            if opt=="Lochness Hunt" and showLochCountdown then showLochCountdown()
            elseif hideLochCountdown then hideLochCountdown() end
        end
    })
    ...
    televent:Dropdown({
        Title="3. Idle Area",
        Values=AreaNames,
        Callback=function(opt) IdleArea = opt end
    })

    ----------------------------------------------------------------
    -- ⑨ SISTEM UTAMA (Smart Event Loop)
    ----------------------------------------------------------------
    televent:Toggle({
        Title="Start Smart Event System",
        Desc="Prioritas → Normal → Idle",
        Value=false,
        Callback=function(state)
            SmartEventState = state

            if state then
                WindUI:Notify({Title="Smart Event",Content="System Started",Duration=3,Icon="cpu"})
                    ...
                    while SmartEventState do
                        ...
                        if SelectedPriorityEvent and SelectedPriorityEvent~="" then
                            if CheckLocalPresence(SelectedPriorityEvent) then
                                priorityFound=true
                                task.wait(WAIT_FARM)
                            elseif ProbeAndFindEvent(SelectedPriorityEvent) then
                                priorityFound=true
                                task.wait(WAIT_SCAN)
                            end
                        end

                        -- 2️⃣ NORMAL EVENT SCAN
                        if not priorityFound and #SelectedNormalEvents>0 then
                            if CurrentCheckIndex>#SelectedNormalEvents then CurrentCheckIndex=1 end
                            local target = SelectedNormalEvents[CurrentCheckIndex]
                            if CheckLocalPresence(target) then
                                normalFound=true
                                task.wait(WAIT_FARM)
                            elseif ProbeAndFindEvent(target) then
                                normalFound=true
                                task.wait(WAIT_SCAN)
                            else
                                CurrentCheckIndex += 1
                ...
                if SmartEventThread then pcall(task.cancel, SmartEventThread) SmartEventThread=nil end
                if WalkOnWaterToggleElement and WalkOnWaterToggleElement.Set then WalkOnWaterToggleElement:Set(false) end
                WindUI:Notify({Title="Smart Event",Content="System Stopped",Duration=3,Icon="x"})
            end
        end
    })
end
