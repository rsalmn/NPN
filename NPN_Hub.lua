...
do
    ...
    local function FindWormParticleEffects(centerPos, radius)
        ...
        -- Scan for particle emitters and effects
        local function scanForParticles(obj)
            if obj:IsA("ParticleEmitter") then
                local distance = (obj.Parent.Position - centerPos).Magnitude
            ...
            -- Check for Attachment effects (common in water events)
            if obj:IsA("Attachment") then
                local distance = (obj.Parent.Position - centerPos).Magnitude
                if distance <= radius then
                    for _, child in ipairs(obj:GetChildren()) do
                        if child:IsA("ParticleEmitter") or child:IsA("Beam") then
                            print("    🎯 Found effect on Attachment:", child.Name)
            ...
            -- Check for water-related models
            if obj:IsA("Model") then
                local name = obj.Name:lower()
                local distance = obj.PrimaryPart and (obj.PrimaryPart.Position - centerPos).Magnitude or 999
                
                if distance <= radius then
                    -- Check for water/effect models that might not have obvious names
                    if name:find("effect") or name:find("water") or name:find("ripple") or
                       name:find("splash") or name:find("wave") or name == "" then
                        
                        -- Count particle emitters in this model
                        local modelParticles = 0
                        for _, desc in ipairs(obj:GetDescendants()) do
                            if desc:IsA("ParticleEmitter") then
                                modelParticles = modelParticles + 1
                            end
                        end
                        
                        if modelParticles > 0 then
                            print("    🌊 Found effect model:", obj.Name, "with", modelParticles, "particles")
                            table.insert(foundParticles, {
                                type = "Effect Model",
                                name = obj.Name,
                                particles = modelParticles,
                                position = obj.PrimaryPart and obj.PrimaryPart.Position or centerPos,
                                distance = distance
                            })
                            waterEffects = waterEffects + modelParticles
                        end
                    end
                end
            end
            ...
            for _, child in ipairs(obj:GetChildren()) do
                scanForParticles(child)
            end
        end
        
        -- Scan workspace for effects
        scanForParticles(workspace)
        
        print("  📊 [WORM] Particle scan results:")
        print("     Total Particles:", particleCount)
        print("     Water Effects:", waterEffects)
        print("     Spiral Effects:", spiralEffects)
        print("     Found Effects:", #foundParticles)
        
        -- Worm events typically have 3+ water effects in a circular pattern
        local isWormEvent = waterEffects >= 3 or (particleCount >= 5 and spiralEffects >= 1)
        
        if isWormEvent then
            print("  ✅ [WORM] Particle signature matches worm event!")
            
            -- Return the center of particle effects as position
            if #foundParticles > 0 then
                local avgPos = Vector3.new(0, 0, 0)
                for _, particle in ipairs(foundParticles) do
                    avgPos = avgPos + particle.position
                end
                avgPos = avgPos / #foundParticles
                
                -- Create a dummy model for caching
                local dummyModel = Instance.new("Model")
                dummyModel.Name = "WormHunt_Particles"
                dummyModel.Parent = workspace
                
                local dummyPart = Instance.new("Part")
                dummyPart.Name = "WormCenter"
                dummyPart.Position = avgPos
    ...
    function ActiveEventsCache:Remove(eventName)
        if self.events[eventName] then
            print("🔓 [CACHE] Unlocked event:", eventName)
            
            -- Clean up dummy models for particle-detected events
            local cached = self.events[eventName]
            if cached.model and cached.model.Name == "WormHunt_Particles" then
                pcall(function()
                    cached.model:Destroy()
                end)
                print("🗑️ [CACHE] Cleaned up particle dummy model")
            end
    ...
    function ActiveEventsCache:IsEventStillActive(eventName)
        ...
        if cached.model and cached.model.Name == "WormHunt_Particles" then
            ...
            local stillActive, _, _ = FindWormParticleEffects(cached.position, 300)
            
            if not stillActive then
                cached.failedChecks = (cached.failedChecks or 0) + 1
                
                if cached.failedChecks >= 3 then
                    print("⚠️ [CACHE] Worm particle event confirmed inactive after", cached.failedChecks, "checks")
                    self:Remove(eventName)
                    return false
                else
                    print("⚠️ [CACHE] Worm particles check failed (", cached.failedChecks, "/3)")
                    return true
                end
            else
