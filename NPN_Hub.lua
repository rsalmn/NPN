...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event System (PRIORITY + SCAN MODE)", TextSize = 20 })
    ...
    local SAFE_LAND_POSITION = Vector3.new(0, 15, 0) -- fallback aman
    ...
    local function CheckExist(eventName)
        if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            return false
        end
        local myPos = LocalPlayer.Character.HumanoidRootPart.Position
        local keywords = EventKeywords[eventName] or {eventName}
                ...
                if part then
                    ...
                    if dist < 1200 then
                        for _, key in ipairs(keywords) do
                            if string.find(string.lower(obj.Name), string.lower(key)) and not Players:GetPlayerFromCharacter(obj) then
    ...
    local function ScanActiveEvents(eventPool)
        local activeList = {}
        for _, name in ipairs(eventPool) do
            local pos = CheckExist(name)
            if pos then
                activeList[name] = pos
            end
        end
    ...
    local function ProcessPriority(name)
        local pos = CheckExist(name)
        if pos then
            WindUI:Notify({Title="PRIORITY ACTIVE", Content="Farming "..name, Duration=3})
            TP(pos)
            repeat
                task.wait(1)
            until not CheckExist(name) or not SystemActive
            WindUI:Notify({Title="PRIORITY DONE", Content=name.." selesai!", Duration=3})
            return true
        end
        return false
    end

    local function ProcessNormalEvents(activeList)
        local activeKeys = {}
        for k,_ in pairs(activeList) do table.insert(activeKeys,k) end

        if #activeKeys == 0 then return false end
        if #activeKeys == 1 then
            local ev = activeKeys[1]
            TP(activeList[ev])
            WindUI:Notify({Title="STAY", Content="Event "..ev.." aktif", Duration=3})
            repeat task.wait(1) until not CheckExist(ev) or not SystemActive
            return true
        else
            WindUI:Notify({Title="MULTI EVENT", Content="Ada "..#activeKeys.." event aktif!", Duration=3})
            for _, ev in ipairs(activeKeys) do
                if not SystemActive then break end
                local pos = activeList[ev]
                TP(pos)
                WindUI:Notify({Title="GOTO", Content="Menuju "..ev, Duration=2})
                local detectAlive = CheckExist(ev)
                local stayTime = detectAlive and 10 or 3
                task.wait(stayTime)
            end
            return true
        end
    ...
    televent:Toggle({
        Title = "Start Smart Auto Farm",
        Desc = "Cek → Prioritas → Scan aktif → Idle",
        Value = false,
        Callback = function(state)
            SystemActive = state
            if state then
                SystemThread = task.spawn(function()
                    while SystemActive do
                        ...
                        -- 1️⃣ Cek Priority Event
                        if SelectedPriority and SelectedPriority ~= "" then
                            farming = ProcessPriority(SelectedPriority)
                        end

                        -- 2️⃣ Jika Priority tidak aktif, cek Normal Events
                        if not farming and #SelectedNormal > 0 then
                            local activeList = ScanActiveEvents(SelectedNormal)
                            if next(activeList) then
                                farming = ProcessNormalEvents(activeList)
                            end
                        end

                        -- 3️⃣ Jika tidak ada event aktif → Idle
                        if not farming then
                            GoIdle()
                            task.wait(5)
                        else
                ...
                if SystemThread then pcall(task.cancel, SystemThread) SystemThread = nil end
            end
        end
    })
end
