...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (V5 FIXED)", TextSize = 20 })
    ...
    -- FIX: More comprehensive keywords
    local EventKeywords = {
        ["Megalodon Hunt"] = {"mega", "lodon", "shark", "fish", "creature", "monster"},
    ...
    -- =========================================================
    -- CRITICAL FIX: IMPROVED DETECTION LOGIC
    -- =========================================================
    
    -- Helper: Check if object matches any keyword
    local function MatchesKeywords(name, keywords)
        ...
        for _, keyword in ipairs(keywords) do
            if lowerName:find(keyword:lower()) then
                print("    ✓ Matched keyword:", keyword, "in", name)
                return true
            end
        end
        return false
    ...
    -- Helper: Advanced alive check
    local function IsEventAlive(obj)
        if not obj or not obj.Parent then return false end
        
        -- Check 1: Humanoid health
        if obj:IsA("Model") then
            local humanoid = obj:FindFirstChildOfClass("Humanoid")
            if humanoid then
                print("    → Found Humanoid, Health:", humanoid.Health)
                if humanoid.Health > 0 then
                    return true
                end
            end
            
            -- Check 2: NumberValue health
            local health = obj:FindFirstChild("Health")
            if health and health:IsA("NumberValue") then
                print("    → Found Health Value:", health.Value)
                if health.Value > 0 then
                    return true
                end
            end
            
            -- Check 3: Has important parts (for treasure events)
            if obj:FindFirstChild("Treasure") or obj:FindFirstChild("Chest") or obj:FindFirstChild("Crate") then
                print("    → Found treasure container")
                return true
            end
        end
        
        -- Check 4: If it's a Part, check parent
        if obj:IsA("BasePart") then
            ...
            -- Some events are just parts without models
            if obj.Anchored == false then -- Moving parts = likely alive
                print("    → Found unanchored part (likely active event)")
                return true
            end
        end
    ...
    -- MAIN FIX: Complete rewrite of scanning logic
    local function DeepScan(position, keywords, targetName)
        print("🔍 [DEEP SCAN] Starting at:", position)
        print("🔍 Looking for keywords:", table.concat(keywords, ", "))
        
        local foundCandidates = {}
        
        -- ============================================================
        -- PASS 1: GetPartBoundsInBox (Best for performance)
        -- ============================================================
        local function ScanWithRadius(radius)
            local params = OverlapParams.new()
            params.FilterDescendantsInstances = {Players.LocalPlayer.Character, workspace.Terrain}
            params.FilterType = Enum.RaycastFilterType.Exclude
            ...
            for _, part in ipairs(parts) do
                ...
                local parentName = parent and parent.Name or "NoParent"
                
                -- Debug: Print every part we find
                if MatchesKeywords(partName, keywords) or MatchesKeywords(parentName, keywords) then
                    print("  🎯 Candidate found:")
                    print("    Part:", partName)
                    print("    Parent:", parentName)
                    print("    Type:", part.ClassName)
                    
                    -- Store candidate for checking
                    local model = part:FindFirstAncestorOfClass("Model")
                    if model then
                        foundCandidates[model] = true
                    else
                        foundCandidates[part] = true
                    end
                end
            end
        end
        ...
        for _, radius in ipairs({200, 400, 600}) do
            ScanWithRadius(radius)
            if next(foundCandidates) then break end -- Stop if we found something
        end
        
        -- ============================================================
        -- PASS 2: Check all candidates
        -- ============================================================
        print("🔍 Checking", table_count(foundCandidates), "candidates...")
        
        for candidate, _ in pairs(foundCandidates) do
            print("  🔬 Testing candidate:", candidate.Name)
            if IsEventAlive(candidate) then
                print("  ✅ ALIVE EVENT CONFIRMED:", candidate.Name)
                
                -- Get position
                local eventPos = nil
                if candidate:IsA("Model") and candidate.PrimaryPart then
                    eventPos = candidate.PrimaryPart.Position
                elseif candidate:IsA("BasePart") then
                    eventPos = candidate.Position
                end
                
                return true, eventPos
            else
                print("  ❌ Candidate is not alive")
            end
        end
        
        -- ============================================================
        -- PASS 3: Workspace direct scan (fallback)
        -- ============================================================
        print("🔍 Trying workspace direct scan...")
        
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj ~= Players.LocalPlayer.Character and obj:IsA("Model") then
                -- Check distance
                ...
                if distance <= 800 then
                    if MatchesKeywords(obj.Name, keywords) then
                        print("  🎯 Workspace candidate:", obj.Name, "Distance:", math.floor(distance))
                        if IsEventAlive(obj) then
                            print("  ✅ WORKSPACE FOUND:", obj.Name)
                            return true, objPos
                        end
                    end
                end
        ...
        -- ============================================================
        -- PASS 4: Check tags (for special events)
        -- ============================================================
        ...
        if success then
            for _, obj in ipairs(tagged) do
                if MatchesKeywords(obj.Name, keywords) then
                    print("  🎯 Tagged candidate:", obj.Name)
                    if IsEventAlive(obj) then
                        print("  ✅ TAGGED FOUND:", obj.Name)
        ...
        print("❌ [DEEP SCAN] No alive events found")
        return false, nil
    end
    ...
    local function ProbeAndFindEvent(targetName)
        ...
        for i, pos in ipairs(coords) do
            ...
            print("✅ Teleported to spot #" .. i)
            
            -- Wait for chunk
            local loaded = WaitForChunkLoad(pos, 3)
            print(loaded and "✅ Chunk loaded" or "⏳ Chunk load timeout")
            
            -- DEEP SCAN with new logic
            local found, eventPos = DeepScan(pos, keywords, targetName)
            
            if found then
                print("✅✅✅ [SUCCESS] Found " .. targetName .. " at spot #" .. i)
                
                -- Move closer if event is far
                if eventPos and (eventPos - pos).Magnitude > 40 then
                    print("📍 Moving closer to event...")
    ...
    function EventQueue:GetNext()
        ...
        for _, event in ipairs(self.queue) do
            ...
            if timeSinceCheck >= event.checkCooldown then
                if timeSinceCheck < oldestTime then
                    best = event
                    oldestTime = timeSinceCheck
                end
            end
        end
    ...
    -- =========================================================
    televent:Toggle({
        Title = "🚀 Enable Smart Event (FIXED)",
        Desc = "Deep Scan Detection System",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart System", 
                    Content = "Fixed Detection Started", 
                    Duration = 3 
                })
                
                print("\n🚀 === SMART EVENT FIXED VERSION STARTED ===")
    ...
    task.spawn(function()
        while true do
            if SmartEventState then
                statsText:Set({
                    Title = "📊 System Status",
                    Desc = string.format("Active | Errors: %d | TPs: %d", 
                        ErrorHandler.ErrorCount, 
                        TeleportManager.TeleportCount)
                })
            else
                statsText:Set({Title = "📊 System Status", Desc = "Offline"})
            end
            task.wait(5)
        end
    end)

    print("✅ [V5 FIXED] Deep scan system loaded!")
end
