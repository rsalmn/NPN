...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (WORM PARTICLE FIXED)", TextSize = 20 })
    ...
    function getLochNextTimes()
        local timeToNext = LochnessSystem:GetNextTime()
        local isActive, status = LochnessSystem:GetStatus()
        return timeToNext, status, isActive
    end
    ...
    -- =========================================================
    -- 🌊 WORM PARTICLE DETECTION SYSTEM
    -- =========================================================
    -- =========================================================
-- 🌊 FIXED WORM PARTICLE DETECTION SYSTEM
-- =========================================================
    local function FindWormParticleEffects(centerPos, radius)
        ...
        -- Scan workspace for effects
        for _, obj in ipairs(workspace:GetDescendants()) do
            local success = pcall(function()
                -- Check ParticleEmitter
                if obj:IsA("ParticleEmitter") and obj.Parent and obj.Parent:IsA("BasePart") then
                    ...
                    if distance <= radius then
                        ...
                        -- Check for water/worm related names
                        ...
                        print("    🎭 Found ParticleEmitter:", obj.Name, "on", obj.Parent.Name)
                        print("       Distance:", math.floor(distance), "| Water Effect:", isWaterEffect)
                    ...
                    if distance <= radius then
                        for _, child in ipairs(obj:GetChildren()) do
                            if child:IsA("ParticleEmitter") or child:IsA("Beam") or child:IsA("Trail") then
                                print("    🎯 Found effect on Attachment:", obj.Name, "->", child.Name)
                                spiralEffects = spiralEffects + 1
                                
                                table.insert(foundParticles, {
                                    type = "Attachment Effect",
                                    name = child.Name,
                                    parent = obj.Parent.Name,
                                    position = partPos,
                                    distance = distance,
        ...
        print("  📊 [WORM] Particle scan results:")
        print("     Total Particles Found:", particleCount)
        print("     Water Effects:", waterEffects)
        print("     Spiral/Attachment Effects:", spiralEffects)
        print("     Valid Particles:", #foundParticles)
        
        -- Worm events have multiple particle effects (usually 5+)
        local isWormEvent = #foundParticles >= 5 or (waterEffects >= 3 and spiralEffects >= 2)
        
        print("  🎯 [WORM] Is Worm Event?", isWormEvent)
        print("     Criteria: foundParticles >= 5 OR (waterEffects >= 3 AND spiralEffects >= 2)")
        
        if isWormEvent and #particlePositions > 0 then
            print("  ✅✅✅ [WORM] CONFIRMED - Particle signature matches worm event!")
            
            -- Calculate center position from all particles
            ...
            print("  📍 [WORM] Calculated center position:", avgPos)
            
            -- Create marker model
            local markerModel = Instance.new("Model")
            markerModel.Name = "WormHunt_ParticleDetection"
            ...
            print("  🎯 [WORM] Created marker model at:", avgPos)
            print("  ✅ [WORM] Returning SUCCESS with position and model")
            
            -- Store particle references in model for verification
            markerModel:SetAttribute("ParticleCount", #foundParticles)
            markerModel:SetAttribute("WaterEffects", waterEffects)
            markerModel:SetAttribute("SpiralEffects", spiralEffects)
            markerModel:SetAttribute("DetectionTime", tick())
            
            return true, avgPos, markerModel
        else
            print("  ❌ [WORM] Not enough particles for worm event")
            print("     Need: 5+ total particles OR (3+ water + 2+ spiral)")
            print("     Got:", #foundParticles, "total |", waterEffects, "water |", spiralEffects, "spiral")
            return false, nil, nil
        end
    end

    -- =========================================================
    -- ACTIVE EVENTS CACHE SYSTEM
    -- =========================================================
    ...
    function ActiveEventsCache:Remove(eventName)
        if self.events[eventName] then
            ...
            -- Clean up particle detection marker models
            if cached.model then
                if cached.model.Name == "WormHunt_ParticleDetection" then
                    pcall(function()
                        print("🗑️ [CACHE] Destroying particle marker model")
...
function ActiveEventsCache:Clear()
    print("🗑️ [CACHE] Clearing all events...")
    
    -- Clean up all marker models
    ...
    self.events = {}
    print("✅ [CACHE] All events cleared")
end
    ...
    function ActiveEventsCache:Clear()
        -- Clean up any dummy models
        for eventName, data in pairs(self.events) do
            if data.model and data.model.Name == "WormHunt_Particles" then
                pcall(function()
                    data.model:Destroy()
                end)
            end
    ...
    function ActiveEventsCache:IsEventStillActive(eventName)
        ...
        if cached.model and cached.model.Name == "WormHunt_ParticleDetection" then
            ...
            -- Re-scan for particles at the cached location
            local stillActive, newPos, _ = FindWormParticleEffects(cached.position, 400)
            
            if stillActive then
                print("✅ [CACHE] Worm particles still detected!")
                cached.failedChecks = 0
                
                -- Update position if particles moved
                if newPos and (newPos - cached.position).Magnitude > 50 then
                    print("📍 [CACHE] Worm position updated:", newPos)
                    cached.position = newPos
                    if cached.model.PrimaryPart then
                        cached.model.PrimaryPart.Position = newPos
                    end
                end
                ...
                cached.failedChecks = (cached.failedChecks or 0) + 1
                print("⚠️ [CACHE] Worm particle check failed (", cached.failedChecks, "/3)")
                
                if cached.failedChecks >= 3 then
                    print("❌ [CACHE] Worm event confirmed inactive (no particles)")
                    self:Remove(eventName)
                    return false
                end
        ...
        -- Normal model existence check (for other events)
        local success, stillExists = pcall(function()
            if not cached.model then return false end
            
            if not cached.model.Parent then
                return false
            end
            
            if cached.model:IsA("Model") then
                if not cached.model:IsDescendantOf(workspace) then
                    return false
                end
            ...
            if cached.model:IsA("BasePart") then
                if not cached.model:IsDescendantOf(workspace) then
                    return false
                end
            end
        ...
        if not success or not stillExists then
            ...
            if cached.failedChecks >= 3 then
                print("⚠️ [CACHE] Event", eventName, "confirmed inactive after", cached.failedChecks, "checks")
                self:Remove(eventName)
                return false
            else
                print("⚠️ [CACHE] Event", eventName, "check failed (", cached.failedChecks, "/3)")
                return true
            end
        end
    ...
    function ActiveEventsCache:ShouldScanForNewEvents()
        ...
        if next(self.events) == nil then
            print("📡 [CACHE] No events cached - initiating scan")
            return true
        end
        
        if timeSinceLastScan >= self.scanInterval then
            print("📡 [CACHE] Scan interval reached (" .. math.floor(timeSinceLastScan) .. "s) - initiating scan")
            return true
        end
        
        print("⏳ [CACHE] Next scan in " .. math.floor(self.scanInterval - timeSinceLastScan) .. " seconds")
        return false
    end
    ...
    function ActiveEventsCache:GetNextEventToVisit()
        local now = tick()
        local oldest = nil
        local oldestTime = math.huge
        
        for eventName, data in pairs(self.events) do
            if self:IsEventStillActive(eventName) then
                local timeSinceVisit = now - data.lastVisit
                if timeSinceVisit < oldestTime then
                    oldest = {name = eventName, data = data}
                    oldestTime = timeSinceVisit
                end
            end
        end
    ...
    function ActiveEventsCache:MarkVisited(eventName)
        if self.events[eventName] then
            ...
            print("✅ [CACHE] Marked", eventName, "as visited")
        end
    end
    ...
    -- =========================================================
    -- 🔧 ENHANCED PROBE FUNCTION WITH WORM PARTICLE DETECTION
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        ...
        print("\n🕵️ [PROBE START] " .. targetName)
        
        -- LOCHNESS
        if targetName == "Lochness Hunt" then
            local isLochActive, status = LochnessSystem:GetStatus()
            
            print("🕒 [LOCHNESS] Status:", status)
            
            if not isLochActive then
                print("⏰ [LOCHNESS] Not in active window - skipping scan")
                return false, nil, nil
            end
            
            print("✅ [LOCHNESS] In active window - proceeding with scan")
                        ...
                        if name:find("nessie") or name:find("loch") then
                            if IsEventAlive(obj) then
                                ...
                                if posSuccess and pos then
                                    print("🎯 [LOCHNESS] Found active Nessie!")
                                    return {true, pos, obj}
                                end
                            end
                        end
        ...
        if targetName == "Worm Hunt" or targetName == "Ghost Worm" then
            ...
            print("🌊 [WORM] Using particle detection method...")
            
            for i, pos in ipairs(coords) do
                print("\n📍 Checking worm spot #" .. i .. " for " .. targetName)
                
                local tpSuccess = TeleportManager:Teleport(pos)
                if not tpSuccess then
                    print("  ❌ Teleport failed to spot #" .. i)
                    task.wait(1)
                    continue
                end
                
                print("  ✅ Teleported successfully to spot #" .. i)
                task.wait(2) -- Wait a bit longer for particles to load
                
                -- Try particle detection first
                local foundParticles, particlePos, particleModel = FindWormParticleEffects(pos, 500)
                
                if foundParticles then
                    print("🎯 [WORM] Particle detection SUCCESS at spot #" .. i)
                    return true, particlePos, particleModel
                end
                
                -- Fallback to normal keyword detection
                print("  🔄 [WORM] Falling back to keyword detection...")
                
                local keywords = EventKeywords[targetName] or {}
                
                for _, radius in ipairs({200, 400, 600}) do
                    local scanSuccess, parts = pcall(function()
                        ...
                        return workspace:GetPartBoundsInBox(
                            CFrame.new(pos), 
                            Vector3.new(radius, 100, radius), 
                            params
                        )
                    end)
                    
                    if scanSuccess and parts then
                        print("  🔍 Radius " .. radius .. " found " .. #parts .. " parts")
                        
                        for _, part in ipairs(parts) do
                            local checkSuccess, eventFound = pcall(function()
                                local partName = part.Name
                                local parent = part.Parent
                                local parentName = parent and parent.Name or ""
                                
                                local partMatch, partKeyword = MatchesKeywords(partName, keywords)
                                local parentMatch, parentKeyword = MatchesKeywords(parentName, keywords)
                                
                                if partMatch or parentMatch then
                                    local matchedKeyword = partKeyword or parentKeyword
                                    local matchSource = partMatch and partName or parentName
                                    
                                    print("    ✓ Matched keyword:", matchedKeyword, "in", matchSource)
                                    ...
                                    if parent and parent:IsA("Model") and IsEventAlive(parent) then
                                        eventModel = parent
                                        print("    ✅✅✅ [WORM EVENT CONFIRMED]", parent.Name)
                                        
                                        local modelPosSuccess, modelPos = pcall(function()
                                            if parent.PrimaryPart then
                                                return parent.PrimaryPart.Position
                                            else
                                                local cf, size = parent:GetBoundingBox()
                                                return cf.Position
                                            end
                                        end)
                                        
                                        if modelPosSuccess and modelPos then
                                            eventPosition = modelPos
                                        else
                                            eventPosition = part.Position
                                        end
                                        
                                    elseif IsEventAlive(part) then
                                        eventModel = part
                                        eventPosition = part.Position
                                        print("    ✅✅✅ [WORM PART CONFIRMED]", partName)
                                    else
                                        print("    ⚠️ Match found but event not alive")
                                        return nil
                                    end
                            ...
                            if checkSuccess and eventFound then
                                print("🎯 [WORM] Keyword detection SUCCESS at spot #" .. i)
                                return true, eventFound.position, eventFound.model
                            end
                        end
                    end
                end
            ...
            print("❌ [WORM] Not found:", targetName)
            return false, nil, nil
        end
        ...
        local keywords = EventKeywords[targetName] or {}
        if #keywords == 0 then return false, nil, nil end

        for i, pos in ipairs(coords) do
            print("\n📍 Checking spot #" .. i .. " for " .. targetName)
                    ...
                    for _, part in ipairs(parts) do
                        local checkSuccess, eventFound = pcall(function()
                            ...
                            if partMatch or parentMatch then
                                ...
                                if parent and parent:IsA("Model") and IsEventAlive(parent) then
                                    eventModel = parent
                                    print("    ✅✅✅ [EVENT CONFIRMED]", parent.Name)
                                ...
                                elseif IsEventAlive(part) then
                                    eventModel = part
                                    eventPosition = part.Position
                                    print("    ✅✅✅ [EVENT PART CONFIRMED]", partName)
                                else
                                    print("    ⚠️ Match found but event not alive")
                                    return nil
                                end
                        ...
                        if checkSuccess and eventFound then
                            print("🎯 [FOUND] Event confirmed at spot #" .. i)
                            return true, eventFound.position, eventFound.model
                        end
                    end
                end
    ...
    local function GetAvailableEvents(eventsToCheck)
        ...
        for _, eventInfo in ipairs(eventsToCheck) do
            ...
            if eventName == "Lochness Hunt" then
                local isActive = LochnessSystem:IsActive()
                if isActive then
                    print("✅ [FILTER] " .. eventName .. " is available (active window)")
                    table.insert(availableEvents, eventInfo)
                else
                    local _, status = LochnessSystem:GetStatus()
                    print("⏰ [FILTER] " .. eventName .. " not available (" .. status .. ")")
                end
            else
                print("✅ [FILTER] " .. eventName .. " is available (normal event)")
                table.insert(availableEvents, eventInfo)
            end
        end
    ...
    televent:Dropdown({
        ...
        Callback = function(opt)
            ...
            if opt == "Lochness Hunt" then 
                if showLochCountdown then showLochCountdown() end 
            else 
                if hideLochCountdown then hideLochCountdown() end 
            end
        end
    })
    ...
    televent:Toggle({
        Title = "🌊 Smart Event (WORM PARTICLE FIXED)",
        Desc = "Enhanced Worm Hunt detection using particle effects",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with Worm particle detection", 
                    Duration = 4 
                })
                
                print("\n🌊 === SMART EVENT SYSTEM (WORM PARTICLE FIXED) STARTED ===")
                
                pcall(function() 
                    if EventTP and EventTP.Stop then EventTP.Stop() end 
                end)
                    ...
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            if hasActiveEvents then
                                print("\n🔒 [LOCKED MODE] Rotating between active events...")
                                
                                local nextEvent = ActiveEventsCache:GetNextEventToVisit()
                                if nextEvent then
                                    print("📍 Visiting:", nextEvent.name, "at", nextEvent.data.position)
                                    TeleportManager:Teleport(nextEvent.data.position)
                                ...
                                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                                    ...
                                    pcall(function()
                                        ...
                                        if hrp and (hrp.Position - data.Pos).Magnitude > 60 then 
                                            print("📍 Going to idle area:", Loch_Return_SelectedArea)
                            ...
                            print("\n📡 [SCANNING MODE] Looking for available events...")
                            ...
                            if #availableEvents == 0 then
                                print("⏰ [FILTER] No events available right now")
                                return
                            end
                            
                            print("🎯 [FILTER] Will scan " .. #availableEvents .. " available events")
                            ...
                            for _, eventInfo in ipairs(availableEvents) do
                                ...
                                print("🎯 [SCAN RESULT]", eventName, "- Found:", found and "YES" or "NO")
                                
                                if found and position and model then
                                    print("✅ SUCCESS! Caching:", eventName)
                            ...
                            if not foundAnyEvent then
                                print("\n❌ No events found - entering idle mode")
                            else
                                print("\n✅ Events cached! Entering locked mode")
                            end
                        end)
    ...
    task.spawn(function()
        while true do
            if SmartEventState then
                ...
                for name, data in pairs(ActiveEventsCache:GetAll()) do
                    cachedCount = cachedCount + 1
                    local failedChecks = data.failedChecks or 0
                    local isParticleEvent = data.model and data.model.Name == "WormHunt_Particles"
                    table.insert(cachedNames, name .. (failedChecks > 0 and " ⚠️" .. failedChecks or "") .. (isParticleEvent and " 🌊" or ""))
                end
                ...
                local status = cachedCount > 0 and "🔒 LOCKED" or "📡 SCANNING"
                
                statsText:Set({
                    Title = "🌊 Smart Event System - " .. status,
                    Desc = string.format("Events Cached: %d\nNext Scan: %dm %ds\nLochness: %s\nErrors: %d | TPs: %d\n%s", 
                        cachedCount,
                        ...
                        cachedCount > 0 and "Active: " .. table.concat(cachedNames, ", ") or "Idle Mode")
                })
            else
    ...
    print("🌊 [SMART EVENT] Loaded with Worm particle detection!")
end
