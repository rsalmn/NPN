...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (EVENT ALIVE FIXED)", TextSize = 20 })
    ...
    function ActiveEventsCache:Add(eventName, position, model)
        self.events[eventName] = {
            ...
            foundAt = tick(),
            lastVisit = 0,
            failedChecks = 0 -- Track consecutive failed checks
        }
    ...
    function ActiveEventsCache:IsEventStillActive(eventName)
        ...
        local success, stillExists = pcall(function()
            ...
            -- Check if model has Parent (still in workspace)
            if not cached.model.Parent then
                return false
            end
            
            -- For Models, check if they're in workspace
            if cached.model:IsA("Model") then
                if not cached.model:IsDescendantOf(workspace) then
                    return false
                end
                
                -- Check if model has essential parts (not destroyed)
        ...
        if not success or not stillExists then
            ...
            -- Only remove after 3 consecutive failed checks (reduce false positives)
            if cached.failedChecks >= 3 then
                ...
                return true -- Still consider active, might be temporary issue
            end
        end
        
        -- Reset failed checks if successful
    ...
    function ActiveEventsCache:MarkVisited(eventName)
        if self.events[eventName] then
            ...
            self.events[eventName].failedChecks = 0 -- Reset on successful visit
    ...
    -- Simplified IsEventAlive - just check if object exists
    local function IsEventAlive(obj)
        if not obj then return false end
        
        local success = pcall(function()
            -- Just check if it has a Parent and is in workspace
    ...
    televent:Toggle({
        Title = "🔧 Smart Event (EVENT ALIVE FIXED)",
        Desc = "Improved event detection with 3-check confirmation",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with improved event detection", 
                    Duration = 4 
                })
                
                print("\n🔧 === SMART EVENT SYSTEM (EVENT ALIVE FIXED) STARTED ===")
        ...
        while true do
            if SmartEventState then
                ...
                for name, data in pairs(ActiveEventsCache:GetAll()) do
                    ...
                    table.insert(cachedNames, name .. (failedChecks > 0 and " ⚠️" .. failedChecks or ""))
                end
    ...
    print("🔧 [SMART EVENT] Loaded with improved event alive detection!")
end
