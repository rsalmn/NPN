...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (CACHED + LOCKED)", TextSize = 20 })
    ...
    local ActiveEventsCache = {
        events = {}, -- {eventName = {position = Vector3, model = Instance, foundAt = tick()}}
        lastFullScan = 0,
        scanInterval = 300, -- 5 minutes (300 seconds)
        lockDuration = 60, -- Stay at event for at least 60 seconds
    }
    ...
    function ActiveEventsCache:IsEventStillActive(eventName)
        ...
        -- Check if model still exists and is alive
    ...
    function ActiveEventsCache:ShouldScanForNewEvents()
        ...
        -- Only scan if:
        -- 1. No events are cached, OR
        -- 2. It's been more than scanInterval seconds since last full scan
    ...
    -- =========================================================
    -- MAIN PROBE FUNCTION (RETURNS POSITION + MODEL)
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        ...
        if targetName == "Lochness Hunt" then
            local success, position, model = pcall(function()
        ...
        for i, pos in ipairs(coords) do
                    ...
                    for _, part in ipairs(parts) do
                        local checkSuccess, checkResult = pcall(function()
                            ...
                            if partMatch or parentMatch then
                                ...
                                print("    ✓ Matched keyword:", matchedKeyword)
                                
                                local model = part:FindFirstAncestorOfClass("Model") or (parent and parent:IsA("Model") and parent)
                                
                                if model and IsEventAlive(model) then
                                    print("    ✅✅✅ [EVENT CONFIRMED]", model.Name)
                                    
                                    local modelPos = pcall(function()
                                        return model.PrimaryPart and model.PrimaryPart.Position or part.Position
                                    end)
                                    
                                    return true, modelPos or pos, model
                                elseif IsEventAlive(part) then
                                    print("    ✅✅✅ [EVENT PART CONFIRMED]", partName)
                                    return true, part.Position, parent or part
                                end
                            end
                        ...
                        if checkSuccess and checkResult then
                            local success, position, model = checkResult, nil, nil
                            
                            -- Extract returned values properly
                            if type(checkResult) == "table" then
                                success = checkResult[1]
                                position = checkResult[2]
                                model = checkResult[3]
                            end
            ...
            if foundMatch then
                return true, foundPosition, foundModel
            end
    ...
    televent:Toggle({
        Title = "🔒 Smart Event (CACHED + LOCKED)",
        Desc = "Locks to found events, scans every 5min",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with caching system", 
                    Duration = 4 
                })
                
                print("\n🔒 === SMART EVENT SYSTEM (CACHED) STARTED ===")
                    ...
                    while SmartEventState do
                        local success, err = pcall(function()
                            ...
                            if hasActiveEvents then
                                ...
                                -- Rotate between active events
                                local nextEvent = ActiveEventsCache:GetNextEventToVisit()
                                if nextEvent then
                                    print("📍 Visiting:", nextEvent.name)
                            ...
                            for _, eventInfo in ipairs(eventsToCheck) do
                                ...
                                if found and position and model then
                                    print("✅ Found and cached:", eventName)
                            ...
                            -- After scanning, check if we found anything
                            if next(ActiveEventsCache:GetAll()) == nil then
                                print("\n❌ No events found - entering idle mode")
                            else
                                print("\n✅ Events found and cached!")
                            end
                        end)
    ...
    print("🔒 [SMART EVENT] Loaded with caching & locking system!")
end
