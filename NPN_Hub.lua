...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event V6 (IMPROVED PROBE)", TextSize = 20 })
    ...
    -- =========================================================
    -- 2. V6 IMPROVED SYSTEM COMPONENTS
    -- =========================================================
    ...
    function TeleportManager:SafeTeleport(pos, options)
        options = options or {}
        ...
        -- Don't teleport if too close
        if self.LastPosition and (finalPos - self.LastPosition).Magnitude < 25 then
            return false, "Too close to last position"
        end
    ...
    function ChunkLoader:WaitForLoad(position, timeout)
        ...
        while tick() - startTime < timeout do
            ...
            if currentCount == lastPartCount then
    ...
    -- ===== OPTIMIZED LOCHNESS DETECTOR =====
    local LochnessDetector = {
        LastScan = 0,
        ScanCooldown = 8,
        CachedPosition = nil,
        KnownSpawnZones = {}
    ...
    function LochnessDetector:IsLochness(obj)
        if not obj then return false end
        local name = obj.Name:lower()
        return name:find("nessie") or name:find("lochness") or name:find("loch")
    end

    function LochnessDetector:Find()
        local now = tick()
        
        if self.CachedPosition and (now - self.LastScan) < self.ScanCooldown then
        ...
        -- Check known spawn zones first
        for _, zone in ipairs(self.KnownSpawnZones) do
            local params = OverlapParams.new()
            params.FilterDescendantsInstances = {Players.LocalPlayer.Character}
            params.FilterType = Enum.RaycastFilterType.Exclude
            ...
            if success then
                for _, part in ipairs(parts) do
                    if self:IsLochness(part) then
                        self.CachedPosition = part.Position
        ...
        local success, tagged = pcall(function()
            return CollectionService:GetTagged("Boss")
        end)
        
        if success then
            for _, obj in ipairs(tagged) do
                if self:IsLochness(obj) then
                    self.CachedPosition = obj.Position
                    table.insert(self.KnownSpawnZones, obj.Position)
                    return obj.Position
                end
            end
        end
        
        -- Limited workspace scan (only children, not descendants)
        local workspaceChildren = workspace:GetChildren()
        for _, obj in ipairs(workspaceChildren) do
            if obj:IsA("Model") and self:IsLochness(obj) then
                local pos = obj:GetPivot().Position
                self.CachedPosition = pos
                table.insert(self.KnownSpawnZones, pos)
                return pos
            end
        end
    ...
    function EventScanner:IsEventAlive(model, eventName)
        if eventName == "Treasure Event" then
            return model:FindFirstChild("Treasure") or 
                   model:FindFirstChild("Chest") or 
                   model:FindFirstChild("Crate")
        end
    ...
    function EventScanner:SmartScan(position, keywords, targetName)
        local radii = {80, 150, 300}
        ...
        for _, radius in ipairs(radii) do
            ...
            -- First pass: Check tagged parts (fastest)
            for _, part in ipairs(parts) do
                if part:HasTag("Event") or part:HasTag("Boss") or part:HasTag("Monster") then
                    local model = part:FindFirstAncestorOfClass("Model")
                    if model and self:IsEventAlive(model, targetName) then
                        return true, model.PrimaryPart and model.PrimaryPart.Position or part.Position
                    end
                end
            end
            
            -- Second pass: Name-based detection
            for _, part in ipairs(parts) do
                local nameLower = part.Name:lower()
                local parentName = part.Parent and part.Parent.Name:lower() or ""
                
                for pattern, _ in pairs(patterns) do
                    if nameLower:find(pattern) or parentName:find(pattern) then
                        local model = part:FindFirstAncestorOfClass("Model")
                        if model and self:IsEventAlive(model, targetName) then
                            return true, model.PrimaryPart and model.PrimaryPart.Position or part.Position
                        end
            ...
            task.wait(0.1) -- Small delay between radius checks
        end
    ...
    local EventQueue = {
        ...
        lastCheck = {},
        spawnPatterns = {
            ["Shark Hunt"] = {interval = 300, duration = 120},
            ["Megalodon Hunt"] = {interval = 600, duration = 180},
            ["Worm Hunt"] = {interval = 400, duration = 150},
            ["Ghost Shark Hunt"] = {interval = 450, duration = 160},
            ["Ghost Worm"] = {interval = 400, duration = 150},
            ["Meteor Rain"] = {interval = 500, duration = 200},
        }
    }

    function EventQueue:Init(events)
        self.queue = {}
        self.lastCheck = {}
    ...
    function EventQueue:GetNextEvent()
        local now = tick()
        local best = nil
        local bestScore = -math.huge
        
        for _, event in ipairs(self.queue) do
            local timeSinceCheck = now - (self.lastCheck[event.name] or 0)
            
            if timeSinceCheck < event.checkCooldown then
                continue
            end
            
            local score = timeSinceCheck * event.priority
            
            -- Boost priority near expected spawn time
            if self.spawnPatterns[event.name] then
                local pattern = self.spawnPatterns[event.name]
                local timeSinceSpawn = now - event.lastSeen
                if timeSinceSpawn >= pattern.interval * 0.85 then
                    score = score * 1.8
                end
            end
        ...
        if best then
            self.lastCheck[best.name] = now
        end
        
        return best
    end

    function EventQueue:MarkFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.lastSeen = tick()
                event.checkCooldown = 8
                break
            end
        end
    end

    function EventQueue:MarkNotFound(eventName)
        for _, event in ipairs(self.queue) do
            if event.name == eventName then
                event.checkCooldown = 30
                break
            end
        end
    end
    ...
    function ErrorHandler:Handle(err)
        ...
        table.insert(self.ErrorLog, self.LastError)
        
        if #self.ErrorLog > 5 then
            table.remove(self.ErrorLog, 1)
        end
        
        warn("⚠️ Smart Event Error #" .. self.ErrorCount .. ":", err)
        
        if self.ErrorCount >= self.MaxErrors then
            WindUI:Notify({
                Title = "System Error",
                Content = "Too many errors (" .. self.ErrorCount .. "), stopping system",
                Duration = 8,
                Icon = "alert-triangle"
            })
            return false
    ...
    function ErrorHandler:Reset()
        self.ErrorCount = 0
        task.delay(180, function() -- Reset every 3 minutes
            self.ErrorCount = math.max(0, self.ErrorCount - 2)
        end)
    end
    ...
    function PerformanceMonitor:StartProbe()
        self.ProbeStartTime = tick()
    end

    function PerformanceMonitor:EndProbe(success, eventName)
        local duration = tick() - self.ProbeStartTime
        
        self.Metrics.TotalProbes = self.Metrics.TotalProbes + 1
        
        local totalTime = self.Metrics.AverageProbeTime * (self.Metrics.TotalProbes - 1)
        self.Metrics.AverageProbeTime = (totalTime + duration) / self.Metrics.TotalProbes
        
        if success then
            self.Metrics.SuccessfulProbes = self.Metrics.SuccessfulProbes + 1
            self.Metrics.EventsFound[eventName] = (self.Metrics.EventsFound[eventName] or 0) + 1
        else
            self.Metrics.FailedProbes = self.Metrics.FailedProbes + 1
        end
    end

    function PerformanceMonitor:GetStats()
        local successRate = (self.Metrics.SuccessfulProbes / math.max(1, self.Metrics.TotalProbes)) * 100
        local uptime = tick() - self.Metrics.StartTime
        local uptimeMin = math.floor(uptime / 60)
        
        return string.format(
            "Uptime: %dm | Probes: %d | Success: %.1f%% | Avg: %.1fs",
            uptimeMin,
            self.Metrics.TotalProbes,
            successRate,
            self.Metrics.AverageProbeTime
        )
    end

    -- =========================================================
    -- 3. MAIN SYSTEM VARIABLES
    -- =========================================================
    ...
    local EventKeywords = {
        ["Megalodon Hunt"] = {"Megalodon", "Mega"},
        ["Shark Hunt"] = {"Shark", "Great White", "Jaws"}, 
        ["Ghost Shark Hunt"] = {"Ghost", "Phantom"},
        ["Worm Hunt"] = {"Worm", "Sandworm"},
        ["Ghost Worm"] = {"Ghost", "Phantom", "Worm"},
        ["Treasure Event"] = {"Chest", "Crate", "Supply", "Treasure"},
        ["Meteor Rain"] = {"Meteor", "Asteroid"},
    }

    -- =========================================================
    -- 5. IMPROVED PROBE FUNCTION
    -- =========================================================
    local function ProbeAndFindEvent(targetName)
        if not targetName then return false end
        
        PerformanceMonitor:StartProbe()
        
        -- LOCHNESS SPECIAL HANDLING
        if targetName == "Lochness Hunt" then
            local pos = LochnessDetector:Find()
            if pos then
                local success, reason = TeleportManager:SafeTeleport(pos)
                if success then
                    print("✅ [Lochness] Found and teleported!")
                    PerformanceMonitor:EndProbe(true, targetName)
                    return true
                else
                    print("⚠️ [Lochness] Found but teleport failed:", reason)
                end
            else
                -- Teleport to safe land to wait
                if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                    local data = FishingAreass[Loch_Return_SelectedArea]
                    TeleportManager:SafeTeleport(data.Pos)
                end
                print("⏳ [Lochness] Waiting for spawn...")
            end
            PerformanceMonitor:EndProbe(false, targetName)
            return false
        end
        ...
        if not coords then 
            print("⚠️ [System] No coordinates for " .. targetName)
            PerformanceMonitor:EndProbe(false, targetName)
            return false 
        end
        ...
        print("🕵️ [V6 Probe] Checking: " .. targetName)

        for i, pos in ipairs(coords) do
            -- 1. SAFE TELEPORT
            local tpSuccess, tpReason = TeleportManager:SafeTeleport(pos)
            if not tpSuccess then
                print("⚠️ [Teleport] Failed:", tpReason)
                if tpReason == "Rate limit exceeded" then
                    task.wait(2) -- Wait if rate limited
                end
                continue
            end
            
            -- 2. WAIT FOR CHUNK LOADING
            if not ChunkLoader:WaitForLoad(pos, 2.5) then
                print("⏳ [Chunk] Loading timeout at spot #" .. i)
            end
            
            -- 3. SMART SCAN
            local found, eventPos = EventScanner:SmartScan(pos, keywords, targetName)
            
            if found then
                print("✅ [FOUND] " .. targetName .. " at spot #" .. i)
                
                -- Re-adjust to exact event position if found
                if eventPos and (eventPos - pos).Magnitude > 20 then
                    TeleportManager:SafeTeleport(eventPos)
                end
        ...
        print("❌ [NOT FOUND] " .. targetName .. " - All spots checked")
        PerformanceMonitor:EndProbe(false, targetName)
        return false
    end
    ...
    -- 6. SYSTEM MANAGER
    -- =========================================================
    local SmartEventSystem = {
        State = false,
        Thread = nil
    }

    function SmartEventSystem:Cleanup()
        if self.Thread then
        ...
        self.State = false
        print("🧹 [V6] System cleaned up")
    end

    function SmartEventSystem:Start()
        if self.State then return end
        
        self:Cleanup()
        self.State = true
        SmartEventState = true
        
        -- Initialize components
        if #SelectedNormalEvents > 0 then
            eventQueue = EventQueue
            eventQueue:Init(SelectedNormalEvents)
        end
        
        ErrorHandler:Reset()
        PerformanceMonitor.Metrics.StartTime = tick()
        
        WindUI:Notify({ 
            Title = "Smart Event V6", 
            Content = "System started with improved probe logic", 
            Duration = 4, 
            Icon = "cpu" 
        })
        
        -- Enable WalkOnWater if available
        pcall(function() 
        ...
        self.Thread = task.spawn(function()
            while self.State and SmartEventState do
                local success, err = pcall(function()
                    local foundActive = false
                    
                    -- [PHASE 1] PRIORITY EVENT CHECK
                    if SelectedPriorityEvent and SelectedPriorityEvent ~= "" then
                        if ProbeAndFindEvent(SelectedPriorityEvent) then
                            ...
                            task.wait(4) -- Hold position at priority event
                        end
                    end

                    if not foundActive then
                        -- [PHASE 2] SMART QUEUE ROTATION
                        if eventQueue and #SelectedNormalEvents > 0 then
                            local nextEvent = eventQueue:GetNextEvent()
                            if nextEvent then
                                if ProbeAndFindEvent(nextEvent.name) then
                                    foundActive = true
                                    eventQueue:MarkFound(nextEvent.name)
                                    print("⚔️ [Event] Farming:", nextEvent.name)
                                    task.wait(6) -- Farm event
                                else
                                    eventQueue:MarkNotFound(nextEvent.name)
                                    task.wait(0.5) -- Brief pause before next check
                                end
                            else
                                task.wait(1) -- No events ready to check
                            end
                        end
                    end
                    ...
                    if not foundActive then
                        ...
                        if Loch_Return_SelectedArea and FishingAreass[Loch_Return_SelectedArea] then
                            ...
                            if hrp then
                                ...
                                if dist > 40 then 
                                    local success = TeleportManager:SafeTeleport(data.Pos)
                                    if success then
                                        print("💤 [Idle] Returned to:", Loch_Return_SelectedArea)
                                    end
                                end
                            end
                        end
                        task.wait(3) -- Idle pause
                    end
                end)
                
                if not success then
                    if not ErrorHandler:Handle(err) then
                        break -- Too many errors, stop system
                    end
                    task.wait(3) -- Error recovery pause
                else
                    -- Decay error count on successful iterations
                    ErrorHandler.ErrorCount = math.max(0, ErrorHandler.ErrorCount - 1)
                end
                
                task.wait(0.2) -- Main loop delay
            end
            
            print("🛑 [V6] Main loop ended")
        end)
    end

    function SmartEventSystem:Stop()
        ...
        WindUI:Notify({ 
            Title = "Smart Event V6", 
            Content = "System stopped and cleaned up", 
            Duration = 3, 
            Icon = "x" 
        })
    end
    ...
    televent:Dropdown({
        ...
        Description = "High priority event (checked first)",
        Values = eventsList,
        AllowNone = true,
        Callback = function(opt)
            ...
            print("🌟 Priority set:", opt or "None")
        end
    })

    televent:Dropdown({
        Title = "⚔️ Normal Events",
        Description = "Events to rotate through (smart queue)",
        Values = eventsList,
        Multi = true,
        Callback = function(opts) 
            SelectedNormalEvents = opts or {} 
            print("⚔️ Normal events:", table.concat(SelectedNormalEvents, ", "))
        end
    })

    televent:Dropdown({
        Title = "🏝️ Idle Area",
        Description = "Area to return when no events active",
        Values = AreaNamess,
        Callback = function(opt)
    ...
    televent:Toggle({
        Title = "🚀 Enable Smart Event V6",
        Description = "Advanced probe system with error handling & optimization",
        Value = false,
        Callback = function(state)
            if state then
                SmartEventSystem:Start()
            else
                SmartEventSystem:Stop()
            end
        end
    })
    ...
    televent:Divider()
    local statsSection = televent:Section({Title = "📊 Performance Stats"})
    local statsLabel = statsSection:Label({Text = "System not started..."})

    -- Stats updater
    task.spawn(function()
        while true do
            if SmartEventState then
                statsLabel:Set(PerformanceMonitor:GetStats())
            else
                statsLabel:Set("System offline")
            end
            task.wait(3)
        end
    end)
    ...
    -- Error log display
    local errorLabel = statsSection:Label({Text = "No errors"})
    task.spawn(function()
        while true do
            if ErrorHandler.LastError then
                errorLabel:Set("Last Error: " .. ErrorHandler.LastError.message .. " (" .. ErrorHandler.LastError.time .. ")")
            else
                errorLabel:Set("No recent errors")
            end
            task.wait(5)
        end
    end)

    print("✅ [Smart Event V6] Loaded successfully with all improvements!")
end
