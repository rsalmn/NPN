...
-- [[ BLATANT FISHING - FINAL OPTIMIZED VERSION ]] --
do
    -- =========================================================
    -- CENTRALIZED REMOTE REFERENCES
    -- =========================================================
    local FishingRemotes = {
        EquipTool = GetRemote(RPath, "RE/EquipToolFromHotbar"),
        Charge = GetRemote(RPath, "RF/ChargeFishingRod"),
        StartMinigame = GetRemote(RPath, "RF/RequestFishingMinigameStarted"),
        Complete = GetRemote(RPath, "RE/FishingCompleted"),
        Cancel = GetRemote(RPath, "RF/CancelFishingInputs"),
        UpdateState = GetRemote(RPath, "RF/UpdateAutoFishingState"),
        MinigameChanged = GetRemote(RPath, "RE/FishingMinigameChanged")
    }
    ...
    local FishingState = {
        ...
        config = {
            Legit = { speed = 0.05 },
            Normal = { delay = 1.5 },
            BlatantV1 = { completeDelay = 0.72, cancelDelay = 0.28, recastDelay = 0.001 },
            BlatantV2 = { completeDelay = 0.79, cancelDelay = 0.329, recastDelay = 0.0 },
            Hybrid = { completeDelay = 0.75, cancelDelay = 0.25, recastDelay = 0.0 }
        }
    }

    function FishingState:SetMode(newMode)
        if self.currentMode == newMode then return end
        
        print("🔄 [FISHING] Switching from", self.currentMode, "to", newMode)
        ...
        if newMode ~= "None" then
            self:StartMode(newMode)
        end
    end

    function FishingState:StopCurrentMode()
        if self.activeThread then
            task.cancel(self.activeThread)
            self.activeThread = nil
        end
        
        if self.equipThread then
            task.cancel(self.equipThread)
            self.equipThread = nil
        end
        
        -- Reset server state
        pcall(function()
            FishingRemotes.Cancel:InvokeServer()
            FishingRemotes.UpdateState:InvokeServer(false)
        end)
        
        print("✅ [FISHING] Stopped mode:", self.currentMode)
    end

    function FishingState:StartMode(mode)
        if not validateRemotes() then return false end
        
        local config = self.config[mode]
        if not config then 
            print("❌ [FISHING] Unknown mode:", mode)
            return false 
        end
        
        -- Start auto-equip thread for all modes
        self.equipThread = task.spawn(function()
            while self.currentMode == mode do
                pcall(function() 
                    FishingRemotes.EquipTool:FireServer(1) 
                end)
                task.wait(0.5)
            end
        end)
        ...
        if mode == "Legit" then
            self:StartLegitMode(config)
        elseif mode == "Normal" then
            self:StartNormalMode(config)
        elseif mode == "BlatantV1" then
            self:StartBlatantV1(config)
        elseif mode == "BlatantV2" then
            self:StartBlatantV2(config)
        elseif mode == "Hybrid" then
            self:StartHybridMode(config)
        end
        
        print("✅ [FISHING] Started mode:", mode)
        return true
    end
    ...
    function FishingState:StartLegitMode(config)
        ...
        FishingController.FishingRodStarted = function(controller, ...)
            oldRodStarted(controller, ...)
            if self.currentMode == "Legit" then
                self.activeThread = task.spawn(function()
                    while self.currentMode == "Legit" do
    ...
    function FishingState:StartNormalMode(config)
        self.activeThread = task.spawn(function()
            while self.currentMode == "Normal" do
                local ts = os.time() + os.clock()
                pcall(function() FishingRemotes.Charge:InvokeServer(ts) end)
                pcall(function() FishingRemotes.StartMinigame:InvokeServer(-139.6, 0.99) end)
                task.wait(config.delay)
                pcall(function() FishingRemotes.Complete:FireServer() end)
                task.wait(0.3)
    ...
    function FishingState:StartBlatantV1(config)
        -- Set server state
        pcall(function() 
            FishingRemotes.UpdateState:InvokeServer(true) 
        end)
        ...
        local function ProtectedComplete()
            ...
            pcall(function()
                FishingRemotes.Complete:FireServer()
            end)
            return true
        end
        ...
        local function PerformCast()
            local t = tick()
            pcall(function()
                FishingRemotes.Charge:InvokeServer({[1] = t})
            end)
            task.wait(0.008)
            pcall(function()
                FishingRemotes.StartMinigame:InvokeServer(1, 0, t)
            end)
        end

        self.activeThread = task.spawn(function()
            while self.currentMode == "BlatantV1" do
                State.doingCycle = true

                PerformCast()
                task.wait(config.completeDelay)

                if self.currentMode == "BlatantV1" then
                    ProtectedComplete()
                end
        ...
        -- Failsafe listener
        FishingRemotes.MinigameChanged.OnClientEvent:Connect(function()
            if self.currentMode ~= "BlatantV1" then return end
    ...
    function FishingState:StartBlatantV2(config)
        -- Set server state
        pcall(function() 
            FishingRemotes.UpdateState:InvokeServer(true) 
        end)
        ...
        self.activeThread = task.spawn(function()
            while self.currentMode == "BlatantV2" do
                local t = tick()

                -- CAST
                pcall(function()
                    FishingRemotes.Charge:InvokeServer({[5] = t})
                end)
                pcall(function()
                    FishingRemotes.StartMinigame:InvokeServer(5, 0, t)
                end)

                -- COMPLETE
                task.wait(config.completeDelay)
                if self.currentMode ~= "BlatantV2" then break end
        ...
        FishingRemotes.MinigameChanged.OnClientEvent:Connect(function()
            if self.currentMode ~= "BlatantV2" then return end
    ...
    function FishingState:StartHybridMode(config)
        -- Set server state
        pcall(function() 
            FishingRemotes.UpdateState:InvokeServer(true) 
        end)
        ...
        self.activeThread = task.spawn(function()
            ...
            while self.currentMode == "Hybrid" do
                local cycleStart = tick()
                State.cycleCount = State.cycleCount + 1
                
                -- [PHASE 1] CAST WITH TIMING VALIDATION
                local castTime = tick()
                local success1 = pcall(function() 
                    FishingRemotes.Charge:InvokeServer({[1] = castTime}) 
                end)
                
                task.wait(0.005) -- Micro delay for packet ordering
                
                local success2 = pcall(function() 
                    FishingRemotes.StartMinigame:InvokeServer(1, 0, castTime) 
                end)
                
                if not (success1 and success2) then
                    print("⚠️ [HYBRID] Cast failed, cycle:", State.cycleCount)
                    task.wait(0.5) -- Recovery delay
                    continue
                end
                
                -- [PHASE 2] WAIT FOR CATCH
                task.wait(config.completeDelay)
                if self.currentMode ~= "Hybrid" then break end
                
                -- [PHASE 3] COMPLETE WITH COOLDOWN CHECK
                local now = tick()
                if now - State.lastComplete >= State.cooldownTime then
                    ...
                    if success3 then
                        ...
                        print("⚠️ [HYBRID] Complete failed, cycle:", State.cycleCount)
                    end
                end
                
                -- [PHASE 4] WAIT FOR ANIMATION
                task.wait(config.cancelDelay)
                if self.currentMode ~= "Hybrid" then break end
                
                -- [PHASE 5] CANCEL/RESET
                pcall(function() 
                    FishingRemotes.Cancel:InvokeServer() 
                end)
                
                -- [PHASE 6] RECAST DELAY
                ...
                if State.cycleCount % 50 == 0 then
                    print(string.format("🔄 [HYBRID] Cycle %d completed in %.3fs", 
                        State.cycleCount, cycleTime))
                end
            end
            
            print("🛑 [HYBRID] Loop ended. Total cycles:", State.cycleCount)
        end)

        -- Failsafe listener
        FishingRemotes.MinigameChanged.OnClientEvent:Connect(function()
            if self.currentMode ~= "Hybrid" then return end
            
            task.delay(config.completeDelay, function()
                if self.currentMode == "Hybrid" then 
                    pcall(function() FishingRemotes.Complete:FireServer() end) 
                end
                
                task.delay(config.cancelDelay, function()
                    if self.currentMode == "Hybrid" then 
                        pcall(function() FishingRemotes.Cancel:InvokeServer() end) 
                    end
                end)
    ...
    Reg("legit", autofish:Toggle({
        ...
        Callback = function(state)
            if state then
                FishingState:SetMode("Legit")
                WindUI:Notify({
                    Title = "Legit Mode ON",
                    Content = "Safe auto-clicking enabled",
                    Duration = 3,
                    Icon = "check"
                })
            else
                FishingState:SetMode("None")
                WindUI:Notify({ Title = "Legit Mode OFF", Duration = 2 })
            end
        end
    }))
    ...
    Reg("tognorm", autofish:Toggle({
        ...
        Callback = function(state)
            if state then
                FishingState:SetMode("Normal")
                WindUI:Notify({
                    Title = "Normal Mode ON",
                    Content = "Instant fishing enabled",
                    Duration = 3,
                    Icon = "zap"
                })
            else
                FishingState:SetMode("None")
                WindUI:Notify({ Title = "Normal Mode OFF", Duration = 2 })
            end
        end
    }))
    ...
    Reg("v1_complete", v1:Input({
        ...
        Value = tostring(FishingState.config.BlatantV1.completeDelay),
        Placeholder = "0.72",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0.1 then
                FishingState.config.BlatantV1.completeDelay = n
            end
        end
    }))
    ...
    Reg("v1_cancel", v1:Input({
        Title = "Cancel Delay",
        Value = tostring(FishingState.config.BlatantV1.cancelDelay),
        Placeholder = "0.28",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0.1 then
                FishingState.config.BlatantV1.cancelDelay = n
            end
        end
    }))
    ...
    Reg("v1toggle", v1:Toggle({
        ...
        Callback = function(state)
            if state then
                FishingState:SetMode("BlatantV1")
                WindUI:Notify({
                    Title = "Blatant V1 ON",
                    Content = "Stable blatant mode activated",
                    Duration = 4,
                    Icon = "zap"
                })
            else
                FishingState:SetMode("None")
                WindUI:Notify({ Title = "Blatant V1 OFF", Duration = 2 })
            end
        end
    }))
    ...
    Reg("v2_complete", v2:Input({
        Title = "Complete Delay",
        Value = tostring(FishingState.config.BlatantV2.completeDelay),
        Placeholder = "0.79",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0 then
                FishingState.config.BlatantV2.completeDelay = n
            end
        end
    }))
    ...
    Reg("v2_cancel", v2:Input({
        Title = "Cancel Delay",
        Value = tostring(FishingState.config.BlatantV2.cancelDelay),
        Placeholder = "0.329",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0 then
                FishingState.config.BlatantV2.cancelDelay = n
            end
        end
    }))
    ...
    Reg("v2toggle", v2:Toggle({
        ...
        Callback = function(state)
            if state then
                FishingState:SetMode("BlatantV2")
                WindUI:Notify({
                    Title = "Blatant V2 ON",
                ...
                FishingState:SetMode("None")
                WindUI:Notify({ Title = "Blatant V2 OFF", Duration = 2 })
            end
        end
    }))
    ...
    Reg("hybrid_comp", hybrid:Input({
        Title = "Complete Delay",
        Value = tostring(FishingState.config.Hybrid.completeDelay),
        Placeholder = "0.75",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0 then 
                FishingState.config.Hybrid.completeDelay = n 
            end
        end
    }))
    ...
    Reg("hybrid_canc", hybrid:Input({
        Title = "Cancel Delay",
        Value = tostring(FishingState.config.Hybrid.cancelDelay),
        Placeholder = "0.25",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0 then 
                FishingState.config.Hybrid.cancelDelay = n 
            end
        end
    }))
    ...
    Reg("hybrid_recast", hybrid:Input({
        Title = "Recast Delay",
        Value = tostring(FishingState.config.Hybrid.recastDelay),
        Placeholder = "0.0",
        Desc = "Delay before next cast (0 = Spam)",
        Callback = function(v)
            local n = tonumber(v)
            if n and n >= 0 then 
                FishingState.config.Hybrid.recastDelay = n 
            end
        end
    }))
    ...
    Reg("hybrid_toggle", hybrid:Toggle({
        ...
        Callback = function(state)
            if state then
                FishingState:SetMode("Hybrid")
                WindUI:Notify({
                    Title = "Hybrid Mode ON",
                    Content = "Best of V1+V2 combined",
                    Duration = 3,
                    Icon = "zap"
                })
            else
    ...
    local function cleanup()
        FishingState:StopCurrentMode()
        print("🧹 [FISHING] Cleanup completed")
    end

    -- Register cleanup
    if getgenv().fishingCleanup then
        getgenv().fishingCleanup()
    end
    getgenv().fishingCleanup = cleanup
...
do
    farm:Divider()
    local televent = farm:Section({ Title = "Smart Event (ROTATION SYSTEM)", TextSize = 20 })
    ...
    -- =========================================================
    -- ACTIVE EVENTS CACHE (MOVED UP FOR DEPENDENCY)
    -- =========================================================
    ...
    function ActiveEventsCache:Add(eventName, position, model)
        ...
        print("🔒 [CACHE] Locked event:", eventName, "at position:", position)
        print("🔒 [CACHE] Total cached events:", #getTableKeys(self.events))
        
        -- Don't rebuild queue here - let main loop handle it
    end
    ...
    function ActiveEventsCache:Remove(eventName)
        ...
        if self.events[eventName] then
            print("🔓 [CACHE] Unlocking event:", eventName)
            
            local cached = self.events[eventName]
            if cached.model and cached.model.Name == "WormHunt_ParticleDetection" then
                pcall(function()
    ...
    function ActiveEventsCache:Get(eventName)
        if not self.events then
            self.events = {}
            return nil
        end
    ...
    function ActiveEventsCache:Clear()
        ...
        for eventName, data in pairs(self.events) do
            if data.model and data.model.Name == "WormHunt_ParticleDetection" then
                pcall(function()
                    data.model:Destroy()
                end)
            end
    ...
    function ActiveEventsCache:ShouldScanForNewEvents()
        if not self.events then
            self.events = {}
        end
    ...
    function ActiveEventsCache:MarkVisited(eventName)
        ...
        if self.events[eventName] then
            ...
            self.events[eventName].failedChecks = 0
            print("✅ [CACHE] Marked", eventName, "as visited (visit #" .. self.events[eventName].visitCount .. ")")
        end
    end
...
function RotationSystem:ForceNextRotation()
    ...
    function LochnessSystem:GetNextTime()
        local currentTime = os.time()
        local timeSinceStart = currentTime - self.startTime
        local cyclesPassed = math.floor(timeSinceStart / self.interval)
        local nextStartTime = self.startTime + (cyclesPassed + 1) * self.interval
    ...
    function LochnessSystem:IsActive()
        ...
        local isActive = timeInCycle >= 0 and timeInCycle <= self.activeWindow
        local remainingTime = isActive and (self.activeWindow - timeInCycle) or 0
        
        return isActive, remainingTime
    end

    function LochnessSystem:GetStatus()
        local isActive, remainingTime = self:IsActive()
        
        if isActive then
            local minutes = math.floor(remainingTime / 60)
            local seconds = remainingTime % 60
            return true, string.format("ACTIVE - %dm %ds left", minutes, seconds)
        else
            local nextTime = self:GetNextTime()
            local hours = math.floor(nextTime / 3600)
            local minutes = math.floor((nextTime % 3600) / 60)
            return false, string.format("Next in %dh %dm", hours, minutes)
        end
    end

    -- =========================================================
    -- EVENT COORDINATES & KEYWORDS
    -- =========================================================
    local EventCoords = {
        ["Shark Hunt"] = {
        ...
        ["Megalodon Hunt"] = {
            Vector3.new(-1076, -1.3, 1676), 
            Vector3.new(-1191, -1.3, 3597),
...
local function FindWormParticleEffects(centerPos, radius)
    ...
    local waterEffects = 0
    local spiralEffects = 0
    local foundParticles = {}
    local particlePositions = {}
    
    -- SCAN FOR ACTUAL WORM MODELS (not just particles)
    for _, obj in ipairs(workspace:GetDescendants()) do
        local success = pcall(function()
            if obj:IsA("Model") and obj.Name:lower():find("worm") then
                local objPos = obj.PrimaryPart and obj.PrimaryPart.Position or obj:GetBoundingBox().Position
                local distance = (objPos - centerPos).Magnitude
                
                if distance <= radius then
                    print("🐛 [WORM] Found actual worm model:", obj.Name, "at", objPos)
                    return true, objPos, obj
                end
            end
            
            -- EXISTING PARTICLE DETECTION
            if obj:IsA("ParticleEmitter") and obj.Parent and obj.Parent:IsA("BasePart") then
                local partPos = obj.Parent.Position
                local distance = (partPos - centerPos).Magnitude
                ...
                if distance <= radius then
                    ...
                    local isWaterEffect = false
                    
                    if name:find("water") or name:find("splash") or name:find("ripple") or 
                       name:find("wave") or name:find("bubble") or name:find("foam") or
                       name:find("particle") or name:find("effect") or
                       parentName:find("water") or parentName:find("wave") or 
                       parentName:find("particle") or parentName:find("effect") then
                        isWaterEffect = true
                        waterEffects = waterEffects + 1
                    end
                    
                    if isWaterEffect then
                        table.insert(foundParticles, {
                            type = "ParticleEmitter",
                            position = partPos,
                            distance = distance,
                        })
                        table.insert(particlePositions, partPos)
                    end
                end
            end
        end)
        ...
        if success and success == true then
            return success -- Return early if actual worm model found
        end
    end
    
    -- FALLBACK TO PARTICLE DETECTION
    local isWormEvent = #foundParticles >= 3 or (waterEffects >= 2) -- LOWERED THRESHOLD
    
    if isWormEvent and #particlePositions > 0 then
        local avgPos = Vector3.new(0, 0, 0)
        for _, pos in ipairs(particlePositions) do
            avgPos = avgPos + pos
        end
        avgPos = avgPos / #particlePositions
        
        -- CREATE PERSISTENT MARKER
        local markerModel = Instance.new("Model")
        markerModel.Name = "WormHunt_ParticleDetection_Persistent"
        
        local markerPart = Instance.new("Part")
        markerPart.Name = "WormEventCenter"
        markerPart.Position = avgPos
        markerPart.Anchored = true
        markerPart.CanCollide = false
        markerPart.Transparency = 1
        markerPart.Size = Vector3.new(10, 10, 10) -- BIGGER SIZE
        markerPart.Parent = markerModel
        
        -- ADD PERSISTENCE ATTRIBUTES
        markerModel:SetAttribute("ParticleCount", #foundParticles)
        markerModel:SetAttribute("WaterEffects", waterEffects)
        markerModel:SetAttribute("DetectionTime", tick())
        markerModel:SetAttribute("IsPersistent", true)
        markerModel:SetAttribute("LastCheck", tick())
        ...
        pcall(function()
            markerModel:SetAttribute("Protected", true)
        end)
        
        print("🏷️ [WORM] Created persistent marker at:", avgPos)
        return true, avgPos, markerModel
    end
...
function ActiveEventsCache:IsEventStillActive(eventName)
    ...
    if eventName == "Lochness Hunt" then
        local isActive = LochnessSystem:IsActive()
    ...
    -- SPECIAL HANDLING FOR WORM EVENTS
    if eventName == "Worm Hunt" or eventName == "Ghost Worm" then
        if cached.model and cached.model.Name:find("WormHunt_ParticleDetection") then
            -- UPDATE LAST CHECK TIME
            cached.model:SetAttribute("LastCheck", tick())
            
            -- CHECK IF MODEL STILL EXISTS
            if not cached.model.Parent or not cached.model:IsDescendantOf(workspace) then
                print("⚠️ [WORM] Marker model lost, re-scanning area...")
                
                -- TRY TO RE-DETECT AT SAME LOCATION
                local stillActive, newPos, newModel = FindWormParticleEffects(cached.position, 600)
                if stillActive then
                    print("✅ [WORM] Re-detected at same location")
                    cached.model = newModel
                    cached.position = newPos
                    cached.failedChecks = 0
                    return true
                else
                    cached.failedChecks = (cached.failedChecks or 0) + 1
                    if cached.failedChecks >= 5 then -- INCREASED TOLERANCE
                        print("❌ [WORM] Failed multiple checks, removing")
                        self:Remove(eventName)
                        return false
                    end
                    print("⏳ [WORM] Failed check", cached.failedChecks .. "/5")
    ...
    local success, stillExists = pcall(function()
        if not cached.model or not cached.model.Parent then return false end
        if not cached.model:IsDescendantOf(workspace) then return false end
        
        if cached.model:IsA("Model") then
            local hasParts = false
            for _, child in ipairs(cached.model:GetDescendants()) do
    ...
    function TeleportManager:CanTeleport()
        local now = tick()
        if now - self.LastTeleport < self.MinInterval then
            return false, "Cooldown"
        end
        if self.TeleportCount > self.MaxTPPerMinute then
...
local function ProbeAndFindEvent(targetName)
    ...
    local function GetAvailableEvents(eventsToCheck)
        ...
        for _, eventInfo in ipairs(eventsToCheck) do
            local eventName = eventInfo.name
            
            if eventName == "Lochness Hunt" then
                local isActive = LochnessSystem:IsActive()
                if isActive then
                    table.insert(availableEvents, eventInfo)
                end
            else
                table.insert(availableEvents, eventInfo)
            end
    ...
    function ErrorHandler:Handle(err)
        ...
        table.insert(self.ErrorLog, {time = tick(), error = self.LastError})
        
        if #self.ErrorLog > 5 then
            table.remove(self.ErrorLog, 1)
        end
        
        warn("⚠️ Error #" .. self.ErrorCount .. ":", err)
        
        if self.ErrorCount >= self.MaxErrors then
            SmartEventState = false
            warn("🛑 Max errors reached")
            return false
        end
        return true
    end
    ...
    -- =========================================================
    televent:Dropdown({
        Title = "1. Priority Event",
        Values = eventsList,
        AllowNone = true,
        Callback = function(opt)
            SelectedPriorityEvent = opt
            print("🎯 [UI] Priority event set to:", opt)
        end
    })

    televent:Dropdown({
        Title = "2. Normal Events",
        Values = eventsList,
        Multi = true,
        Callback = function(opts) 
            SelectedNormalEvents = opts or {} 
            print("📋 [UI] Normal events set to:", table.concat(SelectedNormalEvents, ", "))
        end
    })

    televent:Input({
        Title = "🔄 Rotation Interval",
        Desc = "Seconds between event switches (5-60)",
        Default = tostring(RotationSystem.interval),
        PlaceholderText = "5-60 seconds",
        ...
        Callback = function(text)
            ...
            if value ~= originalValue then
                WindUI:Notify({
                    ...
                    Content = string.format("Clamped from %d to %d seconds", originalValue, value),
                    Duration = 3
                })
            end
            ...
            WindUI:Notify({
                ...
                Content = string.format("Events will rotate every %d seconds", value),
                Duration = 2
            })
            
            print("🔄 [ROTATION] Interval updated to", value, "seconds via user input")
        end
    })
    ...
    televent:Dropdown({
        ...
        Callback = function(opt)
            ...
            print("🏠 [UI] Idle area set to:", opt)
        end
    })

    televent:Toggle({
        Title = "🔄 Smart Event (ROTATION SYSTEM)",
        Desc = "Auto-rotate between all found events",
        Value = false,
        Callback = function(state)
            ...
            if SmartEventState then
                ...
                WindUI:Notify({ 
                    Title = "Smart Event", 
                    Content = "Started with rotation system", 
                    Duration = 4 
                })
...
SmartEventThread = task.spawn(function()
    ...
    televent:Divider()
    local statsText = televent:Paragraph({
        Title = "🔄 Smart Event (Rotation System)",
        Desc = "System offline"
    })
    ...
    task.spawn(function()
        while true do
            if SmartEventState then
                ...
                local success, allEvents = pcall(function()
                    return ActiveEventsCache:GetAll() or {}
                end)
                ...
                if success and allEvents then
                    for name, data in pairs(allEvents) do
                        ...
                        local visitCount = data.visitCount or 0
                        local isPriority = name == SelectedPriorityEvent
                        table.insert(rotationInfo, name .. (isPriority and " 🎯" or "") .. " (" .. visitCount .. ")")
                    end
                end
                
                local timeToRotation = RotationSystem:GetTimeUntilNextRotation()
                local queueSize = #RotationSystem.rotationQueue
                local currentIndex = RotationSystem.currentEventIndex
                
                local timeToNextScan = math.max(0, ActiveEventsCache.scanInterval - (tick() - ActiveEventsCache.lastFullScan))
                ...
                statsText:SetDesc("Active Events: %d | Queue: %d/%d\nNext Rotation: %ds | Interval: %ds\nNext Scan: %dm %ds\nLochness: %s\nErrors: %d | TPs: %d\n%s", 
                        ...
                        currentIndex,
                        queueSize,
                        math.ceil(timeToRotation),
                        RotationSystem.interval,
                        math.floor(timeToNextScan / 60),
                        math.floor(timeToNextScan % 60),
                        lochActive and "🟢 ACTIVE" or "🔴 " .. lochStatus,
    ...
    print("🔄 [SMART EVENT] Loaded with fixed rotation system!")
end
