--============================================================
-- ROCKHUB REBUILD VERSION — FULL WORKING MENU
-- Player | Legit | Normal | Improved Blatant v6
--============================================================
...
RF_ChargeFishingRod = Remotes:WaitForChild("ChargeFishingRod")
RF_RequestFishingMinigameStarted = Remotes:WaitForChild("RequestFishingMinigameStarted")
RE_FishingCompleted = Remotes:WaitForChild("FishingCompleted")
RF_CancelFishingInputs = Remotes:WaitForChild("CancelFishingInputs")
RF_UpdateAutoFishingState = Remotes:WaitForChild("UpdateAutoFishingState")
RE_EquipToolFromHotbar = Remotes:WaitForChild("EquipToolFromHotbar")
...
function SuppressGameVisuals(active)
    -- dibuat ringan, cuma jaga supaya tidak spam notif
end

----------------------------------------------------------------
-- IMPROVED BLATANT ENGINE V6 (FULL)
----------------------------------------------------------------
...
local function KillThread(threadName)
    if threadName and task.cancel then
        pcall(function()
            task.cancel(threadName)
        end)
    end
end
...
print("RockHub Rebuild Loaded Successfully")
