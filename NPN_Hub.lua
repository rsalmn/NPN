...
----------------------------------------------------------------
-- ROCKHUB IMPROVED BLATANT ENGINE v6
-- Integrated Upgrade (Safer, Faster, Stealth, No Thread Leak)
----------------------------------------------------------------

_G.RockHub_ImprovedBlatant = true
...
local function runImprovedBlatant()
    ...
    if not checkFishingRemotes or not checkFishingRemotes(true) then
        blatantActive = false
        return
    end
    ...
    task.spawn(function()
        ...
        local remaining = (completeDelay or 3) - (os.clock() - start)
        if remaining > 0 then
            task.wait(remaining)
        end
        
        SafeFire(RE_FishingCompleted)
        task.wait(cancelDelay or 0.3)
...
function SetBlatantState(state)
    ...
    if SuppressGameVisuals then
        SuppressGameVisuals(state)
    end

    if state then
        -- guarantee perfection sync
        for i = 1,3 do
...
print("RockHub Improved Blatant v6 Loaded Successfully")
